...
genuine coverage gap existed within my sole-owner `src/gnn/utils/` path: the shared
...
- `tests/utils/test_io_utils.py` (new, 12 tests)
...
- `tests/utils/test_path_conversion.py` (new, 8 tests)
...
- `pytest tests/infrastructure tests/setup tests/template
  tests/pipeline -q --tb=no -x`: **484 passed** in 340s
- `pytest tests/utils/ -q --tb=no -x`: **147 passed** (was 127; +20 new
  edge tests)
- `ruff check src/gnn/utils src/gnn/pipeline src/gnn/setup src/gnn/template`: **All checks
  passed**
- `ruff format --check`: **93 files already formatted** (incl. new test files)

## Note on repo state

`git status` shows unrelated modified/untracked files (`src/gnn/api/*`,
`src/gnn/analysis/*`, `src/gnn/ontology/*`, `output/20_website_output/*`,
`tests/analysis/*`, `_probe_param.py`, `jax_outputs/`) that predate this
