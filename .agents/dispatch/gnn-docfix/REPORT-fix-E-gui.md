...
- FIX 1 (was line 78): Module path `src/gnn/mermaid_converter.py` -> `src/gnn/gui/oxdraw/mermaid_converter.py`
- FIX 2 (was line 355): Module path `src/gnn/mermaid_parser.py` -> `src/gnn/gui/oxdraw/mermaid_parser.py`
...
- FIX 4 (was line 371 and surrounding): Rewrote the fabricated fielded-`ParsedGNN` API to the real dict-based API used by `src/gnn/gui/oxdraw/mermaid_converter.py` / `mermaid_parser.py`:
...
- FIX 5 (was lines 823, 864, 888 and 845, 853, 878): repointed the fabricated `src/oxdraw_integration/` package section to the actual `src/gnn/gui/oxdraw/` integration:
  - `### Module: src/oxdraw_integration/` -> `src/gnn/gui/oxdraw/`
  - `#### File: src/oxdraw_integration/__init__.py` -> `src/gnn/gui/oxdraw/__init__.py`
  - `#### File: src/oxdraw_integration/processor.py` -> `src/gnn/gui/oxdraw/processor.py`
  - Import `from oxdraw_integration.processor import process_oxdraw_gui` -> `from gui.oxdraw.processor import process_oxdraw`; `processing_function=process_oxdraw`; `def process_oxdraw(`; `"process_oxdraw"` in `__all__` (verified `process_oxdraw` is the real function in `src/gnn/gui/oxdraw/processor.py`).

### 2. doc/gui_oxdraw/VERIFICATION.md

- FIX 6 (was line 25): `### Core Implementation (src/oxdraw/)` -> `src/gnn/gui/oxdraw/`
...
- FIX 9 (was lines 197-198): `src/gnn/mermaid_converter.py` / `src/gnn/mermaid_parser.py` -> `src/gnn/gui/oxdraw/mermaid_converter.py` / `src/gnn/gui/oxdraw/mermaid_parser.py`
- FIX 10 (was lines 61, 63-65): corrected test file paths and counts (verified against the tree):
  - `tests/gui/test_oxdraw_integration.py` — 465 lines, 15 test functions
  - `tests/visualization/test_mermaid_converter.py` — 399 lines, 26 test functions
  - `tests/visualization/test_mermaid_parser.py` — 402 lines, 27 test functions
