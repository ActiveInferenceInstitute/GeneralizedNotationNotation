...
- `RxInferRenderer` does not exist, so the RxInfer usage was updated to the real functional
  API `render_gnn_to_rxinfer(model, output_path)` (verified signature in src/gnn/render/__init__.py).
...
- `from src.llm import get_processor` -> `from src.llm import get_global_processor`;
  ...
  the exported alias, verified in src/gnn/llm/__init__.py).
...
- `# src/gnn/security/validator.py` -> annotated illustrative, pointing to the real
  src/gnn/security/processor.py.
- `# src/gnn/llm/security/prompt_sanitizer.py` -> annotated illustrative (src/gnn/llm has no security/ subdir).
- `# src/gnn/mcp/security/secure_server.py` -> annotated illustrative.
- `# tests/security/test_security.py` -> corrected to `# tests/security/test_security_functional.py`.
- `# src/gnn/security/audit.py` -> annotated illustrative (path does not exist).
