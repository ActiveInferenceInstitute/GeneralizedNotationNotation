...
- **Fix 2** (line ~168): `from utils.logging_utils import setup_standalone_logging` → `from utils.logging.logging_utils import setup_standalone_logging` (verified path `src/gnn/utils/logging/logging_utils.py`).
- **Fix 3** (line ~362): pytest path `tests/unit/` → `tests/gnn/`.
- **Fix 4** (line ~369): `tests/performance/ --benchmark-only` → real path `tests/pipeline/ -v`; comment retitled "Run pipeline orchestration tests"; dropped `--benchmark-only`.
- **Fix 5** (line ~471): `tests/unit/test_specific.py` → `tests/gnn/test_gnn_overall.py` (verified file exists).
- **Fix 6** (line ~120): "171 pytest files" → "~323 pytest source files" (verified `find src/tests -name '*.py' | wc -l` = 323).
- **Fix 7** (lines ~287-300 tree; also line ~378 prose): rewrote the test-organization tree to the real layout (`tests/<module>/`, `tests/integration/`, `tests/helpers/`, `tests/test_data/`, `conftest.py`). Also updated the "Add tests" reference from `tests/unit/` and `tests/integration/` to `tests/<module>/` and `tests/integration/`.
...
### `SKILL.md`
- **Fix 10** (line ~25): "Check all 8 renderer backends" → "Check all 9 renderer backends" (verified `src/gnn/render/framework_registry.py` and `src/gnn/render/SPEC.md` list 9: pymdp, rxinfer, activeinference_jl, jax, discopy, pytorch, numpyro, stan, bnlearn).
