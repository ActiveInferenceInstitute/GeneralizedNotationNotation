...
(`src/gnn/utils/arg_parsing.py`), registries (`src/gnn/render/framework_registry.py`,
`src/gnn/execute/executor.py`, `src/gnn/execute/processor.py`), and CLI (`src/gnn/cli/`).
...
the main parser. Evidence: `src/gnn/utils/arg_parsing.py:108` defines
...
`profile`. So `uv run python src/gnn/main.py --profile` is accepted as a performance-profiling
...
But the repo ships a committed environment for it — `src/gnn/execute/activeinference_jl/Project.toml`
...
- doc/HANDOFF.md opens as "Superseded snapshot" and records a point-in-time 2026-07-30 pass
  ...
  (tests/runner.py, src/gnn/llm/providers/base_provider.py, scripts, TO-DO.md, CLAUDE.md) all
...
- All CLI commands in doc/README.md and doc/quickstart.md: `uv run gnn preflight`, `gnn health`,
  ...
  `uv run python src/gnn/main.py`, `uv run python src/gnn/5_type_checker.py`, `python -m json.tool`.
  `gnn` is registered as `src.cli:main` in `pyproject.toml`. The 15-subcommand dispatcher
  (incl. validate/preflight/health/templates/pull) exists in `src/gnn/cli/__init__.py` and
  `src/gnn/cli/templates.py`. Template `actinf-pomdp-2state` exists in `src/gnn/cli/template_assets/`.
...
- 9 render-targets claim: `src/gnn/render/framework_registry.py` has exactly 9 keys (pymdp,
  activeinference_jl, jax, discopy, pytorch, numpyro, stan, bnlearn).
- 8 executor-families claim: `src/gnn/execute/processor.py::parse_frameworks_parameter` returns
  8 families (pymdp, jax, discopy, rxinfer, active_inf.jl, pytorch, numpyro, bnlearn).
- `00_pipeline_summary/pipeline_execution_summary.json` naming matches `src/gnn/pipeline/context.py:215-216`.
- `--only-steps --skip-steps --frameworks --target-dir --output-dir` are real args
  (src/gnn/utils/arg_parsing.py); `input/config.yaml` exists as the auto-loaded config described.
...
- dependencies/* claims: RxInfer "no runtime Pkg.add" guidance matches the committed
  `src/gnn/execute/rxinfer/` Project.toml env.