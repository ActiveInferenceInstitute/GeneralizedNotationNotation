...
def per_file_markdown_report(filename: str, result: Dict[str, Any]) -> str:
    ...
    lines.append(
        f"- **Overall Complexity**: {result.get('model_complexity', {}).get('overall_complexity', 0):.2f}"
    )
    lines.append("")
    ...
    if "variables" in result:
        ...
        for v in result["variables"]:
            lines.append(
                f"| {v['name']} | {v['type']} | {v['dimensions']} | {v.get('total_elements', 'N/A')} |"
            )
        lines.append("")
    ...
    if "connections" in result:
        ...
        for conn in result["connections"]:
            lines.append(
                f"| {conn['source']} | {conn['target']} | {conn['type']} | {'Yes' if conn['is_temporal'] else 'No'} |"
            )
        lines.append("")
...
def summary_markdown_report(all_results: Dict[str, Dict[str, Any]]) -> str:
    ...
    complexities = [
        r.get("model_complexity", {}).get("overall_complexity", 0)
    ...
    for fname, res in all_results.items():
        ...
        lines.append(
            f"| {os.path.basename(fname)} | {status} | {n_err} | {n_warn} | {n_vars} | {n_conn} | {complexity:.2f} | {model_type} |"
        )
...
def variables_table_csv(all_results: Dict[str, Dict[str, Any]]) -> List[List[Any]]:
    ...
    for fname, res in all_results.items():
        for v in res.get("variables", []):
            rows.append(
                [os.path.basename(fname), v["name"], v["type"], v["dimensions"]]
            )
    return rows


def section_presence_matrix_csv(
    all_results: Dict[str, Dict[str, Any]], section_list: List[str]
) -> List[List[Any]]:
