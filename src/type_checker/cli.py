...
def main(cmd_args=None):
    ...
    parser.add_argument(
        "--estimate-resources",
        action="store_true",
        help="Estimate computational resources for the GNN models",
    )
    parser.add_argument(
        "-o",
        "--output-dir",
    ...
    if not allow_any_output_dir:
        ...
        if str(actual_output_dir).rstrip("/\\").lower() in [
            str(Path(d)).lower() for d in forbidden_dirs
        ] or not actual_output_dir.name.lower().endswith("type_check"):
            print(
    ...
    logger.debug(
        f"  Recursive: {parsed_args.recursive}, Strict: {parsed_args.strict}, Estimate Resources: {parsed_args.estimate_resources}"
    )
    ...
    try:
        ...
        logger.error(
            f"An error occurred during GNN checking phase: {e_check}", exc_info=True
        )
        return 1
    ...
    # Summary reports
    write_markdown(
        summary_dir / "type_check_summary.md", summary_markdown_report(details_dict)
    )
    write_json(
        summary_dir / "type_check_summary.json", summary_json_report(details_dict)
    )
    # CSV summary
    write_csv(
        summary_dir / "type_check_summary.csv",
        [
            [k, v["is_valid"], len(v.get("errors", [])), len(v.get("warnings", []))]
    ...
    write_csv(
        artifacts_dir / "variables_table.csv",
        variables_table_csv(details_dict),
        header=["File", "Variable", "Type", "Dimensions"],
    )
    write_csv(
        artifacts_dir / "section_presence_matrix.csv",
        section_presence_matrix_csv(details_dict, all_sections),
    )

    # Additional artifacts
    write_csv(
        artifacts_dir / "connections_table.csv",
        connections_table_csv(details_dict),
        header=["File", "Source", "Target", "Type", "Temporal"],
    )
    write_csv(
        artifacts_dir / "complexity_analysis.csv",
        complexity_analysis_csv(details_dict),
        header=[
            "File",
            "Variables",
            "Connections",
    ...
    write_csv(
        artifacts_dir / "type_distribution.csv",
        type_distribution_csv(details_dict),
        header=["File", "Type", "Count"],
    )

    try:
        report_summary_text = checker.generate_report(
            results,
            actual_output_dir,
            report_md_filename=markdown_report_name,
            project_root_path=parsed_args.project_root,
        )
        logger.info("\n--- Type Check Report Summary ---")
        for line in (
            report_summary_text.splitlines()
        ):  # Log line by line to respect logger formatting
            logger.info(line)
        logger.info("--- End of Type Check Report Summary ---")
        logger.info(
            f"Main type check report saved in: {actual_output_dir / markdown_report_name}"
        )
        ...
        checker.generate_json_data(
            results, type_check_data_json_path
        )  # Call the new public method
        logger.info(f"Detailed JSON data saved in: {type_check_data_json_path}")

    except Exception as e_report:
        logger.error(
            f"An error occurred during report generation for type checking: {e_report}",
            exc_info=True,
        )
    ...
    if parsed_args.estimate_resources:
        ...
        try:
            ...
            logger.error(
                f"Failed to create resource estimates output directory {resource_estimator_output_base_dir}: {e_res_dir}"
            )
            # Don't fail the whole script if only this sub-dir creation fails, but log it.

        type_check_data_json_path = (
            actual_output_dir / "resources" / "type_check_data.json"
        )
        estimator = GNNResourceEstimator(
            type_check_data=str(type_check_data_json_path)
            if type_check_data_json_path.exists()
            else None
        )
        ...
        try:
            if input_path_obj.is_file():
                ...
                estimator.results = {
                    str(input_path_obj): result
                }  # Store for report generation
            elif input_path_obj.is_dir():
                estimator.estimate_from_directory(
                    str(input_path_obj), recursive=parsed_args.recursive
                )
            # else case already handled above for main checker

            resource_report_summary_text = estimator.generate_report(
                str(resource_estimator_output_base_dir),
                project_root_path=parsed_args.project_root,
            )
            ...
            for r_line in summary_lines:
                logger.info(r_line)
            logger.info(
                f"Resource estimation reports (markdown, JSON, HTML) saved in: {resource_estimator_output_base_dir}"
            )
            logger.info("--- End of Resource Estimation Report Summary ---")
        except Exception as e_est:
            logger.error(
                f"An error occurred during resource estimation: {e_est}", exc_info=True
            )
