...
def generate_markdown_report(
    ...
    output_path: Path,
    actual_project_root: Optional[Path] = None,
) -> str:
    ...
    if valid_results:
        avg_memory = sum(r["memory_estimate"] for r in valid_results) / len(
            valid_results
        )
        avg_inference = sum(r["inference_estimate"] for r in valid_results) / len(
            valid_results
        )
        avg_storage = sum(r["storage_estimate"] for r in valid_results) / len(
            valid_results
        )
        ...
        report_content = ["# GNN Resource Estimation Report", ""]
        report_content.append(
            f"Analyzed {total_files} files, but no valid results were obtained."
        )
    ...
    for file_path_str, res in results.items():
        ...
        report_content.append(
            f"Inference Estimate: {res['inference_estimate']:.2f} units"
        )
