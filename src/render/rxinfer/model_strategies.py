...
``FlatStrategy`` preserves the original canonical flat-POMDP generator
verbatim. The remaining kinds are intentional ``NotImplementedError`` stubs
until their generators are implemented upstream.
"""
...
class FactoredStrategy(_NotImplementedStrategy):
    """Multiple independent hidden state factors."""

    kind = ModelKind.FACTORED


class HierarchicalStrategy(_NotImplementedStrategy):
    """Multi-level state hierarchy."""

    kind = ModelKind.HIERARCHICAL


class MultiAgentStrategy(_NotImplementedStrategy):
...
class LearningStrategy(_NotImplementedStrategy):
    """Parameter learning (Dirichlet priors, etc.)."""
