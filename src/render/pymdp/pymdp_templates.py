...
# Fallback utilities for PyMDP functionality
...
def generate_example_usage_template(
    ...
    usage_lines.append(f"{inner_indent}{indent}{indent}s_next[f_idx] = np.random.choice(len(B_gen_process[f_idx][:, s_current[f_idx], action_for_factor]))")
    ...
    usage_lines.append(f"{inner_indent}{indent}{indent}o_next[g_idx] = np.random.choice(len(prob_vector))")
...
def generate_placeholder_matrices(num_modalities: int, num_states: List[int]) -> Dict[str, List[str]]:
    ...
    if num_modalities > 0 and len(num_states) > 0:
        ...
        matrix_defs["A"].append(f"A = obj_array({num_modalities})")
        ...
        for i in range(num_modalities):
            ...
            matrix_defs["A"].append(f"likelihood_{i} = softmax(raw_weights_{i}, axis=0)")  # Normalize across states
        ...
        matrix_defs["A"].append("A = obj_array(1)")
    ...
    if len(num_states) > 0:
        ...
        matrix_defs["B"].append(f"B = obj_array({len(num_states)})")
        ...
        matrix_defs["B"].append("B = obj_array(1)")
    ...
    if num_modalities > 0:
        ...
        matrix_defs["C"].append(f"C = obj_array({num_modalities})")
        ...
        matrix_defs["C"].append("C = obj_array(1)")
    ...
    if len(num_states) > 0:
        ...
        matrix_defs["D"].append(f"D = obj_array({len(num_states)})")
        ...
        matrix_defs["D"].append("D = obj_array(1)")
