...
def generate_placeholder_matrices(num_modalities: int, num_states: List[int]) -> Dict[str, List[str]]:
    """
    Generate placeholder matrix definitions for PyMDP when real data is not available.
    ...
    # A matrix placeholder (likelihood)
    if num_modalities > 0 and len(num_states) > 0:
        matrix_defs["A"].append("# WARNING: A is a minimal placeholder matrix since no A_spec was provided in GNN")
        matrix_defs["A"].append("# This is intended for basic structure only and may not run correctly")
        matrix_defs["A"].append(f"A = utils.obj_array({num_modalities})")
        for i in range(num_modalities):
            matrix_defs["A"].append(f"A[{i}] = utils.norm_dist(np.ones((1, 1))) # Placeholder for modality {i}")
    else:
        matrix_defs["A"].append("A = None # No modalities or states defined")
        
    # B matrix placeholder (transitions)
    if len(num_states) > 0:
        matrix_defs["B"].append("# WARNING: B is a minimal placeholder matrix since no B_spec was provided in GNN")
        matrix_defs["B"].append("# This is intended for basic structure only and may not run correctly")
        matrix_defs["B"].append(f"B = utils.obj_array({len(num_states)})")
        for i in range(len(num_states)):
            matrix_defs["B"].append(f"B[{i}] = utils.norm_dist(np.ones((1, 1, 2))) # Using 2 actions to satisfy PyMDP Agent requirements")
    else:
        matrix_defs["B"].append("B = None # No state factors defined")
    
    # C vector placeholder (preferences)
    if num_modalities > 0:
        matrix_defs["C"].append(f"C = utils.obj_array_zeros({num_modalities}) # Default to no preferences")
    else:
        matrix_defs["C"].append("C = None # No modalities defined")
    
    # D vector placeholder (initial beliefs)
    if len(num_states) > 0:
        matrix_defs["D"].append(f"D = utils.obj_array({len(num_states)}) # Initial beliefs")
        for i in range(len(num_states)):
            matrix_defs["D"].append(f"D[{i}] = utils.norm_dist(np.ones({max(1, num_states[i])})) # Default uniform for factor {i}")
    else:
        matrix_defs["D"].append("D = None # No state factors defined")
