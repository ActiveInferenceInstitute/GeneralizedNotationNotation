...
def _numpy_array_to_string(arr: np.ndarray, indent: Any = 0) -> str:
    ...
    if (arr.ndim == 1) or (
        arr.ndim == 2 and arr.shape[0] <= 3 and arr.shape[1] <= 5
    ):  # Heuristic for small 2D
...
def format_list_recursive(
    ...
    if make_multiline:
        ...
        for i, item in enumerate(data_list):
            ...
            for sub_line in item_lines[
                1:
            ]:  # Subsequent lines of item, already indented by formatter relative to its start
                lines.append(
                    item_indent_str + sub_line
                )  # So, just add the item_indent_str for this block
...
def generate_pymdp_matrix_definition(
    ...
    num_modalities_or_factors: Optional[
        int
    ] = None,  # This param is mostly for comment generation
    ...
    if data is None:
        if is_object_array:
            logger.debug(
                f"Data for object array {matrix_name} is None, defaulting to np.array([], dtype=object)."
            )
            lines.append(f"{matrix_name} = np.array([], dtype=object)")
        else:
            logger.debug(
                f"Data for non-object array {matrix_name} is None, setting to None."
            )
            lines.append(f"{matrix_name} = None")
        return "\n".join(lines)

    if isinstance(data, str) and any(
        util_str in data for util_str in ["pymdp.", "np.", "utils.", "maths."]
    ):
    ...
    if is_object_array and isinstance(data, list):
        ...
        for item_val in data:
            ...
            if isinstance(item_val, np.ndarray):
                ...
                item_str = _numpy_array_to_string(
                    item_val, indent=len(base_indent_str * 2)
                )
            elif isinstance(item_val, (list, tuple)):  # Try to convert to np.array
                try:
                    np_equiv = np.array(item_val)
                    item_str = _numpy_array_to_string(
                        np_equiv, indent=len(base_indent_str * 2)
                    )
                except Exception as e:
                    logger.warning(
                        f"Object array {matrix_name}: Skipping unconvertible list/tuple item: {item_val} ({e})"
                    )
                    continue
            else:  # Non-array, non-list/tuple item: skip as per test expectations
                logger.warning(
                    f"Object array {matrix_name}: Skipping item of type {type(item_val)}: {item_val}"
                )
                continue
        ...
        if not valid_item_strings:
            ...
            lines.append(
                f"{temp_list_name} = [ # Object array for {actual_items_count} modalities/factors"
            )
            for _, s_arr in enumerate(valid_item_strings):
                ...
                lines.append(
                    base_indent_str + s_arr_lines[0] + ","
                )  # First line of "np.array(...)"
                for sub_line in s_arr_lines[1:]:
                    lines.append(
                        base_indent_str + sub_line + ","
                    )  # Subsequent lines of "np.array(...)"
            ...
            if lines and valid_item_strings:  # Ensure there was at least one item
                for i_l in range(len(lines) - 1, -1, -1):
                    ...
                    if (
                        stripped_line == f"{temp_list_name} = ["
                    ):  # Stop if we hit list start
                        break
        ...
        if np_array_data is None:  # If it was a list/tuple, try to convert
            try:
                ...
                logger.error(
                    f"Matrix {matrix_name}: Could not convert list/tuple to np.array: {e}. Assigning None.",
                    exc_info=True,
                )
        ...
        logger.warning(
            f"Matrix {matrix_name}: Unexpected data type {type(data)}. Assigning directly or via repr."
        )
...
def generate_pymdp_agent_instantiation(
    ...
    if control_fac_idx_list is not None:
        all_params["control_fac_idx"] = control_fac_idx_list
    if policy_len is not None:
        all_params["policy_len"] = policy_len
    if use_utility is not None:
        all_params["use_utility"] = use_utility
    if use_states_info_gain is not None:
        all_params["use_states_info_gain"] = use_states_info_gain
    if use_param_info_gain is not None:
        all_params["use_param_info_gain"] = use_param_info_gain
    if action_selection is not None:
        all_params["action_selection"] = (
            action_selection  # String literal for action_selection value
        )
    ...
    if learning_params:
        all_params.update(learning_params)
    if algorithm_params:
    ...
    for key, value in all_params.items():
        ...
        ):  # This specific one needs to be a string literal
    ...
    if action_names is not None:
        lines.append("")
        lines.append("# Set action names as agent attribute")
