...
def get_pipeline_scripts(current_dir: Path) -> list[dict[str, int | str | Path]]:
    ...
    pipeline_scripts_info: list[
        dict[str, int | str | Path]
    ] = []  # Explicitly type hint
    ...
    for script_path_str in all_candidate_files:
        ...
        if match:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"ℹ️ Matched script for pipeline: {script_basename} (Number: {script_num})"
                )

    pipeline_scripts_info.sort(key=lambda x: (x["num"], x["basename"]))
    sorted_script_basenames: list[str] = [
        str(info["basename"]) for info in pipeline_scripts_info
    ]  # Explicitly cast to str and type hint
