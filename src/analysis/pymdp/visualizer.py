...
class PyMDPVisualizer:
    ...
    def plot_action_sequence(
        ...
        if num_actions is None:
            try:
                num_actions = (
                    int(max(action_sequence) + 1) if len(action_sequence) > 0 else 1
                )
            except Exception:
                num_actions = 1
    ...
    def plot_episode_summary(
        ...
        metrics = {
            ...
            "belief_confidence": [
                max(b)
                if hasattr(b, "tolist") or isinstance(b, (list, tuple, np.ndarray))
                else 0
                for b in episode_trace.get("beliefs", [])
            ],
            "cumulative_preference": episode_trace.get("rewards", []),
        }
        fig = self.visualize_performance_metrics(
            metrics, title=f"Episode {episode_num} Summary"
        )
        if fig and save_path:
            Path(save_path).parent.mkdir(parents=True, exist_ok=True)
            fig.savefig(save_path, dpi=300, bbox_inches="tight")
    ...
    def visualize_discrete_states(
        self, states: List[int], num_states: int, title: str = "Discrete State Sequence"
    ) -> Optional[Any]:
        """
        ...
        ax2.bar(
            range(num_states), state_counts, color=self.colors["visited"], alpha=0.7
        )
    ...
    def visualize_belief_evolution(
        self, beliefs: List[np.ndarray], title: str = "Belief Evolution"
    ) -> Optional[Any]:
        """
        ...
        if beliefs_array.ndim == 1:
            ...
            beliefs_array = (
                beliefs_array[:, :, 0]
                if beliefs_array.shape[2] <= beliefs_array.shape[0]
                else beliefs_array[0, :, :]
            )
    ...
    def visualize_performance_metrics(
        ...
        if "cumulative_preference" in metrics:
            axes[1, 1].plot(
                np.cumsum(metrics["cumulative_preference"]),
                "o-",
    ...
    try:
        ...
        if "states" in simulation_results:
            visualizer.visualize_discrete_states(
                ...
                simulation_results.get(
                    "num_states", max(simulation_results["states"]) + 1
                ),
        ...
        if "beliefs" in simulation_results:
            visualizer.visualize_belief_evolution(
                simulation_results["beliefs"], "Belief State Evolution"
            )

        # Visualize performance metrics
        if "metrics" in simulation_results:
            visualizer.visualize_performance_metrics(
                simulation_results["metrics"], "Simulation Performance"
            )
            ...
            if vfe and efe:
                visualizer.visualize_vfe_vs_efe(
                    vfe, efe, "Active Inference Energy Dynamics"
                )
