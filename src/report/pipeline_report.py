...
def _section_header(summary: Dict[str, Any]) -> str:
    ...
    total_duration = summary.get("total_duration", "N/A")
    success = summary.get("success", "unknown")
    status_emoji = "🟢" if success is True else "🔴" if success is False else "⚪"

    return f"""# Pipeline Execution Report

{status_emoji} **Status**: {"SUCCESS" if success is True else "FAILED" if success is False else "UNKNOWN"}  
📅 **Timestamp**: {timestamp}  
...
def _section_step_status(
    ...
    if steps:
        ...
        for step in steps:
            ...
            lines.append(f"| {step.get('step_num', '?')} | {name} | {emoji} {status} | {duration} | {files} | {size} |")
    elif step_stats:
        lines.append("")
...
def _section_timing(summary: Dict[str, Any]) -> str:
    ...
    for step in sorted_steps:
        ...
        lines.append(f"| {step.get('name', '?')} | {dur:.1f} | {bar} {pct:.0f}% |")
