...
```python
from gnn.audio.sapf import SAPFGNNProcessor
...
```python
from pathlib import Path
from gnn.audio import generate_audio_from_gnn, analyze_audio_characteristics
...
```python
from pathlib import Path
from gnn.audio import create_sonification
...
Tests live in `tests/audio/` (generation, edge cases, integration, MCP tools,
streaming, SAPF). Run them with:

```bash
uv run --extra dev python -m pytest tests/audio/ -v
```
