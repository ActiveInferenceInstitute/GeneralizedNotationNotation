...
class SAPFGNNProcessor:
    ...
    def parse_gnn_sections(self, gnn_content: str) -> Dict[str, Any]:
        ...
        state_space_match = re.search(
            r"## StateSpaceBlock\s*\n(.*?)(?=\n## |\n# |\Z)", gnn_content, re.DOTALL
        )
        if state_space_match:
            sections["StateSpaceBlock"] = self._parse_state_space(
                state_space_match.group(1)
            )
        ...
        if connections_match:
            sections["Connections"] = self._parse_connections(
        ...
        if params_match:
            sections["InitialParameterization"] = self._parse_parameters(
    ...
    def convert_to_sapf(self, gnn_sections: Dict[str, Any], model_name: str) -> str:
        ...
        if "StateSpaceBlock" in gnn_sections:
            sapf_lines.extend(
                self._generate_state_oscillators(
                    gnn_sections["StateSpaceBlock"], base_freq
                )
            )
        ...
        if "Connections" in gnn_sections:
            sapf_lines.extend(
                self._generate_connection_routing(gnn_sections["Connections"])
            )

        # Generate matrix processing
        if "InitialParameterization" in gnn_sections:
            sapf_lines.extend(
                self._generate_matrix_processing(
                    gnn_sections["InitialParameterization"]
                )
            )
    ...
    def _get_model_signature(
        self, model_name: str, gnn_sections: Dict[str, Any]
    ) -> int:
    ...
    def _get_model_tempo(self, complexity: str) -> float:
        ...
        return cast("float", tempo_map.get(complexity, 1.0))
    ...
    def _generate_state_oscillators(
        self, state_space: List[Dict[str, Any]], base_freq: float
    ) -> List[str]:
        ...
        for i, state in enumerate(state_space):
            ...
            if total_dims <= 6:
                ...
                lines.append(
                    f"  ; {var_name} ({var_category}, {var_type}, dims={dimensions})"
                )
                ...
                detune = (i * 5) + (
                    len(var_category) % 7
                )  # Stronger category-based detuning
                lines.append(
                    f"  ; {var_name} ({var_category}, dims={dimensions}) harmonic series"
                )
                lines.append("  [")
    ...
    def _generate_connection_routing(
        ...
        for conn in connections:
            ...
            if conn_type == ">":
                ...
                lines.append("  base_signal .1 0 lfsaw .2 * 1 + * = base_signal")
            else:
                # Undirected connection - mixing
                lines.append(f"  ; {source} - {target} (undirected)")
                lines.append("  base_signal dup .5 * + = base_signal")

        lines.extend(["  base_signal", "] = connection_audio", ""])
    ...
    def _generate_matrix_processing(self, params: Dict[str, Any]) -> List[str]:
        ...
        lines.extend(["  signal", "] = matrix_audio", ""])
    ...
    def _generate_temporal_structure(self, time_config: Dict[str, Any]) -> List[str]:
        ...
        lines.extend(["  matrix_audio time_env *", "] = temporal_audio", ""])
...
def generate_audio_from_sapf(
    sapf_code: str, output_file: Path, duration: float = 10.0
) -> bool:
    """
