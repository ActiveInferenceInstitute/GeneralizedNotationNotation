...
### GNN to SAPF Audio
```python
from gnn.audio.sapf import process_gnn_to_audio
...
### GNN to SAPF Code
```python
from gnn.audio.sapf import convert_gnn_to_sapf, validate_sapf_code
...
from gnn.audio.sapf import generate_audio_from_sapf
...
```python
from gnn.audio.sapf import SyntheticAudioGenerator, generate_oscillator_audio, apply_envelope, mix_audio_channels
```
...
### Orchestrated By
- **Parent Module**: `src/gnn/audio/` (Step 15)
...
### Test Files
- `tests/sapf/` - processor, edge-case, and MCP tool tests
- `tests/audio/test_audio_sapf.py` - SAPF tests within the audio suite

### Test Commands
```bash
uv run --extra dev python -m pytest tests/sapf/ tests/audio/test_audio_sapf.py -v

uv run --extra dev python -m pytest tests/sapf/ tests/audio/test_audio_sapf.py \
    --cov=src/gnn/audio/sapf --cov-report=term-missing
```
...
### Tool Endpoints
```python
from gnn.audio.sapf.module_info import register_tools
```
