...
**Example**:
```python
from gnn.integration import process_integration
...
### Basic Usage
```python
from gnn.integration.processor import process_integration
...
- `src/gnn/main.py` - Runs `17_integration.py` as a pipeline step (subprocess)
...
- `tests/integration/test_integration_mcp.py`, `test_integration_mcp_tools.py` - MCP tool tests
- `tests/integration/test_meta_analysis_none_states.py` - Meta-analysis edge cases
...
uv run --extra dev python -m pytest tests/integration/ \
    --cov=src/gnn/integration --cov-report=term-missing
```
