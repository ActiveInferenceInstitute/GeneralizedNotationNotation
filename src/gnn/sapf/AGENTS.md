...
```python
from gnn.sapf import process_gnn_to_audio
...
### SAPF Conversion
```python
from gnn.sapf import convert_gnn_to_sapf
...
```python
from gnn.sapf import generate_audio_from_sapf
...
- `audio_results.json` - Processing results (written by `src/gnn/audio/processor.py`)
...
### Test Files
- `tests/sapf/test_sapf_processor.py` - SAPF processor tests
- `tests/audio/test_audio_sapf.py` - SAPF audio backend tests

### Test Coverage
Measure on demand:

```bash
uv run --extra dev python -m pytest tests/test_sapf*.py \
    --cov=src/gnn/sapf --cov-report=term-missing
```
### Key Test Scenarios
