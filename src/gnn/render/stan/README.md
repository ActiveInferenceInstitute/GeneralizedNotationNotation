...
`src/gnn/render/stan/` renders a GNN model to a **runnable Stan program** plus a
...
```python
from gnn.render.stan import render_gnn_to_stan
...
Execution and the result schema are documented in `src/gnn/execute/stan/`.
