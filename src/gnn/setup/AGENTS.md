...
The thin orchestrator `src/gnn/1_setup.py` defines:
...
```bash
uv run --extra dev python -m pytest tests/setup tests/test_uv_environment.py \
    --cov=src/gnn/setup --cov-report=term-missing
```
