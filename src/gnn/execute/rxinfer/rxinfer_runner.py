...
def execute_rxinfer_script(
    ...
    Args:
        ...
        output_dir: Directory for execution logs. When provided, the
            envelope's stdout/stderr and a structured execution log are
            persisted (jax-style) so RxInfer runs leave debugging evidence.
        timeout: Execution timeout in seconds (default: 300).

    Returns:
        dict with ``success`` (bool), ``script_name``, ``return_code``,
        ``stdout``, ``stderr``, and ``error``/``error_type`` on failure.
    """
    ...
    if script_path.suffix.lower() == ".jl":
        try:
            ...
            return {"success": False, "script_name": script_path.name, "error": str(e)}
    ...
    if script_path.suffix.lower() == ".jl":
        ...
        if not runner_script.exists():
            ...
            return {
                "success": False,
                "script_name": script_path.name,
                "error": "RxInfer.jl runner script not found",
            }
        ...
        return {
            "success": False,
            "script_name": script_path.name,
            "error": f"Unsupported file type: {script_path.suffix}",
        }
    ...
    if envelope["error_type"] == "TimeoutExpired":
        ...
        result["error"] = envelope.get("error", "invocation failed")
        result["error_type"] = envelope.get("error_type", "RuntimeError")
        logger.error(
            f"Error executing script {script_path.name}: {result['error']}"
        )
    ...
    # Persist execution evidence (jax-style): stdout, stderr, and a
    # structured execution log — on success AND failure — so RxInfer runs
    # leave debugging artifacts (wave-2 MED-03b).
    if output_dir is not None:
        try:
            log_dir = Path(output_dir) / script_path.stem
            log_dir.mkdir(parents=True, exist_ok=True)
            (log_dir / f"{script_path.stem}_stdout.txt").write_text(
                result.get("stdout", ""), encoding="utf-8"
            )
            if result.get("stderr", "").strip():
                (log_dir / f"{script_path.stem}_stderr.txt").write_text(
            ...
            if result.get("error"):
                ...
                execution_log["error_type"] = result.get("error_type", "")
            (log_dir / f"{script_path.stem}_execution_log.json").write_text(
                json.dumps(execution_log, indent=2), encoding="utf-8"
            )
            logger.debug(f"RxInfer execution logs saved to: {log_dir}")
        except Exception as log_err:
            logger.warning(f"Could not save RxInfer execution logs: {log_err}")
...
def run_rxinfer_scripts(
    ...
    if execution_output_dir:
        ...
        exec_output_dir = Path(rendered_simulators_dir) / "rxinfer_execution_logs"
        exec_output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for script_file in script_files:
        result = execute_rxinfer_script(
            script_file, verbose, output_dir=exec_output_dir
        )
