...
from gnn.execute.activeinference_jl import run_activeinference_analysis
...
```python
from pathlib import Path
from gnn.execute.activeinference_jl import execute_activeinference_script, find_activeinference_scripts
...
project_dir = Path("src/gnn/execute/activeinference_jl")
...
`src/gnn/render/activeinference_jl/activeinference_renderer.py` is the source of truth
for the schema.
...
### Recovery Strategies
- Run `julia --project=src/gnn/execute/activeinference_jl --startup-file=no src/gnn/execute/activeinference_jl/setup_environment.jl`
...
### Orchestrated By
- **Parent Module**: `src/gnn/execute/` (Step 12)
...
### Data Flow
```
render.activeinference_jl (.jl script) → setup_environment.jl → julia --project=src/gnn/execute/activeinference_jl → simulation_results.json → Step 12 summaries → Step 16 analysis
```
...
### Test Files
- `tests/execute/test_execute_overall.py` - Execute module tests (includes framework selection)
- `tests/pipeline/test_pomdp_gridworld_cross_framework.py` - Cross-framework contract including ActiveInference.jl
- `tests/render/test_activeinference_matrix_formatting.py` - Renderer-side contract for the scripts this module executes

### Test Commands
```bash
uv run --extra dev python -m pytest tests/execute/test_execute_overall.py \
    tests/pipeline/test_pomdp_gridworld_cross_framework.py -v

# With coverage
uv run --extra dev python -m pytest tests/execute/test_execute_overall.py \
    --cov=src/gnn/execute/activeinference_jl --cov-report=term-missing
```
...
(`src/gnn/execute/mcp.py`) exposes `process_execute`, `execute_gnn_model`,
...
### Adding New Features
1. Change the generated Julia in `src/gnn/render/activeinference_jl/` (this module only runs what the renderer emits)
...
5. Add or extend tests in `tests/execute/` and `tests/render/`
