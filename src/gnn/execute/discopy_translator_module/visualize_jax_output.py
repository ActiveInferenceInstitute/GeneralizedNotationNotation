    ...
    try:
        ...
        if len(shape) == 1:
            ...
            slice_data = (
                numpy_data[0]
                if shape[0] > 0
                else numpy_data.reshape(shape[1], shape[2])
            )
...
def plot_multiple_tensor_outputs(
    ...
    for tensor_name, tensor_data in tensor_dict.items():
        try:
            # Sanitize filename
            safe_name = "".join(
                c for c in tensor_name if c.isalnum() or c in ("_", "-")
            )
            output_path = output_directory / f"{prefix}_{safe_name}.png"

            success, message = plot_tensor_output(
                tensor_data, output_path, title=f"JAX Output: {tensor_name}"
            )
    ...
    if success_count == total_count:
        ...
        return (
            True,
            f"Plotted {success_count}/{total_count} tensors (failed: {failed_plots})",
            saved_files,
        )
    else:
...
def create_summary_visualization(
    evaluation_results: Dict[str, Any], output_path: Union[str, Path]
) -> Tuple[bool, str]:
    """
