...
class PipelineScriptValidator:
    ...
    def validate_script(self, script_path: Path) -> ScriptValidationResult:
        ...
        result = ScriptValidationResult(
            script_name=script_path.name, script_path=script_path
        )
    ...
    def _validate_module_imports(self, result: ScriptValidationResult) -> Any:
        ...
        for module_path in result.imported_modules:
            if (
                "." in module_path
                and not module_path.startswith("utils.")
                and not module_path.startswith("pipeline.")
            ):
    ...
    def _check_safe_fail_patterns(
        ...
        has_exit_codes = (
            "return 0" in content and "return 1" in content
        ) or "run_script()" in content
        has_sys_exit = (
            "sys.exit(main())" in content
            or "sys.exit(run_script())" in content
            or "raise SystemExit(main())" in content
            or "raise SystemExit(run_script())" in content
        )
    ...
    def generate_validation_report(
        ...
        if issue_types.get("missing_module", 0) > 0:
            recommendations.append(
                "Create missing module directories and __init__.py files"
            )
        if issue_types.get("missing_function", 0) > 0:
            recommendations.append(
                "Implement missing functions in module __init__.py files"
            )
        ...
        report: dict[str, Any] = {
            ...
            "summary": {
                ...
                "validation_success_rate": (valid_scripts / total_scripts) * 100
                if total_scripts > 0
                else 0,
            },
            "issue_summary": {
                ...
                for issue in result.issues:
                    if issue.issue_type == "missing_module":
                        ...
                        if len(parts) >= 2:
                            ...
                            module_name = (
                                issue.message.split(" in ")[-1]
                                if " in " in issue.message
                                else "unknown"
                            )
                            script_fixes.append(
                                f"Implement required function: {function_name}"
                            )
