...
def process_single_file(input_file: Path, output_dir: Path, options: dict) -> bool:
    """
    ...
    try:
        ...
        if input_file.suffix.lower() == ".md":
            ...
            analysis_results["metadata"] = _extract_structured_metadata(
                content, input_file.suffix
            )
        else:
        ...
        logger.debug(
            f"Generated outputs: {analysis_file}, {processed_file}, {summary_file}"
        )
        return True
...
def _detect_content_type(content: str) -> str:
    ...
    if content.strip().startswith("{") or content.strip().startswith("["):
        ...
        for keyword in ["gnn", "statespaceblock", "connections"]
    ):
...
def _extract_structured_metadata(content: str, file_type: str) -> dict:
    ...
    try:
        ...
        return {"structure_type": "structured", "is_valid": False, "error": str(e)}
    ...
    def _analyze_level(obj: Any, depth: int = 0) -> Dict[str, Any]:
        ...
        if isinstance(obj, dict):
            return {
                ...
                "key_count": len(obj),
                "sample_values": {
                    k: type(v).__name__ for k, v in list(obj.items())[:5]
                },
            }
...
def _generate_processed_content(content: str, analysis: dict) -> str:
    ...
    processed_lines.append(
        f"# Processing Timestamp: {analysis['processing_timestamp']}"
    )
    processed_lines.append(f"# Content Type: {analysis['content_type']}")
    processed_lines.append(
        f"# File Size: {analysis['file_size_bytes']} bytes, {analysis['file_size_lines']} lines"
    )
    processed_lines.append("")
...
def _generate_summary_report(analysis: dict) -> str:
    ...
    if "metadata" in analysis:
        ...
        if metadata.get("structure_type") == "structured":
            report_lines.append(
                f"- **Structure:** {metadata.get('structure_type', 'Unknown')}"
            )
            report_lines.append(f"- **Valid:** {metadata.get('is_valid', 'Unknown')}")
            if "top_level_keys" in metadata:
                report_lines.append(
                    f"- **Top-level Keys:** {', '.join(metadata['top_level_keys'][:10])}"
                )
        elif metadata.get("structure_type") == "text":
            report_lines.append(f"- **Words:** {metadata.get('word_count', 0)}")
            report_lines.append(
                f"- **Characters:** {metadata.get('character_count', 0)}"
            )
            report_lines.append(
                f"- **Non-empty Lines:** {metadata.get('non_empty_lines', 0)}"
            )
            report_lines.append(
                f"- **Average Line Length:** {metadata.get('average_line_length', 0):.1f}"
            )
        elif "headers" in metadata:
            report_lines.append(
                f"- **Headers Found:** {len(metadata.get('headers', []))}"
            )
            report_lines.append(
                f"- **Code Blocks:** {metadata.get('code_blocks_count', 0)}"
            )
...
def main(parsed_args: Any) -> int:
    ...
    if not input_files:
        log_step_warning(
            logger, f"No input files found in {input_dir} using pattern '{pattern}'"
        )
            ...
            try:
                with performance_tracker.track_operation(f"process_{input_file.name}"):
                    success = process_single_file(
                        input_file, step_output_dir, processing_options
                    )
    ...
    logger.info(
        f"Processing complete: {successful_files}/{total_files} files successful"
    )
