...
from gnn.utils import log_step_start, log_step_success, setup_step_logging
...
EXPECTED_OUTPUTS: dict[str, Any] = {
    ...
    "3_gnn": ["gnn_processing_results.json"],
    "4_model_registry": ["model_registry_results.json"],
    "5_type_checker": ["type_check_results.json"],
    "6_validation": ["validation_results.json"],
    "7_export": ["export_results.json"],
    "8_visualization": ["visualization/"],
    "9_advanced_viz": ["advanced_viz_summary.json"],
    "10_ontology": ["ontology_results/"],
    "11_render": [
    ...
    "13_llm": ["llm_results/"],
    "14_ml_integration": ["ml_integration_summary.json"],
    "15_audio": ["audio_processing_summary.json"],
    "16_analysis": ["analysis_results.json"],
    "17_integration": ["integration_summary.json"],
    "18_security": ["security_processing_summary.json"],
    "19_research": ["research_processing_summary.json"],
    "20_website": ["website/"],
    "21_mcp": ["mcp_results/"],
    "22_gui": ["gui_processing_summary.json"],
    "23_report": ["report_summary.json"],
    "24_intelligent_analysis": ["intelligent_analysis_summary.json"],
}
        ...
        for func in log_functions:
            ...
            if re.search(pattern, content):
                issues["suggestions"].append(
                    f"Check {func} calling pattern - may need logger argument"
                )
    ...
    try:
        ...
        if module_path.name.startswith(
            ...
            if "from pipeline" not in content and "pipeline" not in content:
                issues["suggestions"].append(
                    "Consider using centralized pipeline configuration"
                )
        ...
        for pattern in hardcoded_patterns:
            if re.search(pattern, content):
                issues["improvements"].append(
                    "Consider using centralized path configuration"
                )
                break
        ...
        if (
            ...
            and "from utils import" in content
        ):
            issues["improvements"].append(
                "Redundant recovery imports - utils now provides graceful fallbacks"
            )
        ...
        if module_path.name == "main.py":
            if "ArgumentParser" not in content:
                issues["suggestions"].append(
                    "Consider using ArgumentParser for better argument handling"
                )
        elif "__name__ == '__main__'" in content:
            if "argparse.ArgumentParser" in content and "ArgumentParser" not in content:
                issues["suggestions"].append(
                    "Consider using ArgumentParser.parse_step_arguments for consistency"
                )
        ...
        if module_path.name in [
            ...
            if "performance_tracker" not in content:
                issues["suggestions"].append(
                    "Consider adding performance tracking for this compute-intensive step"
                )
...
def validate_configuration_consistency() -> Dict[str, List[str]]:
    ...
    try:
        from gnn.pipeline.config import STEP_METADATA, PipelineConfig
        ...
        if missing_metadata:
            issues["warnings"].append(
                f"Steps missing metadata: {', '.join(missing_metadata)}"
            )

        extra_metadata = metadata_steps - configured_steps
        if extra_metadata:
            issues["warnings"].append(
                f"Metadata for unconfigured steps: {', '.join(extra_metadata)}"
            )
            ...
            for dep in step_config.dependencies:
                if dep not in configured_steps:
                    issues["errors"].append(
                        f"Step {step_name} depends on unconfigured step {dep}"
                    )
                dep_config = config.get_step_config(dep)
                if dep_config and not dep_config.required:
                    issues["warnings"].append(
                        f"Step {step_name} depends on optional step {dep}"
                    )
        ...
        for step_name, step_config in config.steps.items():
            if not step_config.output_subdir:
                issues["suggestions"].append(
                    f"Step {step_name} has no output subdirectory configured"
                )
...
def generate_improvement_recommendations(report: Dict) -> List[str]:
    ...
    if total_modules > 0:
        error_modules = sum(
            1 for issues in module_issues.values() if issues.get("errors")
        )
        warning_modules = sum(
            1 for issues in module_issues.values() if issues.get("warnings")
        )
        ...
        if warning_modules > 0:
            recommendations.append(
                f"🟡 **Improve**: Address warnings in {warning_modules} modules"
            )
            recommendations.append(
                "   - Add missing required imports (log_step_*, setup_step_logging)"
            )
    ...
    if performance_modules:
        recommendations.append(
            "📊 **Performance**: Add performance tracking to compute-intensive steps"
        )
        recommendations.append(
            f"   - Modules to enhance: {', '.join(performance_modules)}"
        )
...
def validate_argument_consistency() -> Dict[str, List[str]]:
    ...
    try:
        ...
        if missing_definitions:
            issues["warnings"].append(
                f"Steps missing argument definitions: {', '.join(missing_definitions)}"
            )
        ...
        for step_name, step_args in STEP_ARGUMENTS.items():
            ...
            if missing_common:
                inconsistent_steps.append(
                    f"{step_name} missing: {', '.join(missing_common)}"
                )
...
def validate_dependency_cycles() -> Dict[str, List[str]]:
    ...
    try:
        from gnn.pipeline.config import PipelineConfig
    ...
    try:
        ...
        for step_name, expected_pattern in expected_patterns.items():
            ...
            if step_config and step_config.output_subdir != expected_pattern:
                violations.append(
                    f"{step_name}: expected '{expected_pattern}', got '{step_config.output_subdir}'"
                )
...
def validate_performance_tracking_coverage() -> Dict[str, List[str]]:
    ...
    try:
        ...
        if missing_tracking:
            ...
            issues["suggestions"].append(
                "Enable performance tracking for compute-intensive steps"
            )
...
def generate_validation_report(src_dir: Path, output_dir: Path) -> Dict:
    ...
    # Generate summary
    total_issues = sum(
        len(issues.get("errors", [])) + len(issues.get("warnings", []))
    ...
    log_step_success(
        logger, f"Validation complete. Status: {report['summary']['status']}"
    )
    return report
...
def main() -> int:
    ...
    parser.add_argument(
        "--save-report", type=Path, help="Save detailed report to JSON file"
    )
    ...
    log_step_success(
        logger, f"Pipeline validation completed with exit code {exit_code}"
    )
