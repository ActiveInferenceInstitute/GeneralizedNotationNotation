...
- **Location**: `src/gnn/0_template.py`, `src/gnn/1_setup.py`, etc.
...
- **Location**: `src/gnn/template/`, `src/gnn/setup/`, `src/gnn/validation/`, etc.
...
```bash
python src/gnn/main.py --target-dir input/gnn_files --output-dir output --verbose
```
...
4. **Create thin orchestrator** `src/gnn/23_new_step.py`:
   ```python
...
def test_template_orchestrator():
    result = subprocess.run([sys.executable, "src/gnn/0_template.py", ...])
...
def test_main_pipeline():
    result = subprocess.run([sys.executable, "src/gnn/main.py", ...])
    assert result.returncode == 0
```
