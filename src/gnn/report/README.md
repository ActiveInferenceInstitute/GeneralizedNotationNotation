...
Tests live in `tests/report/` (integration, generation, formats, functional, diff/ledgers, MCP wrappers).

```bash
uv run --extra dev python -m pytest tests/report/ --cov=src/gnn/report
```
