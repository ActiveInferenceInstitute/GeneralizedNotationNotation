...
```python
from gnn.research.processor import process_research
...
- `tests/research/test_research_mcp_tools.py` - MCP tool tests
- `tests/research/test_research_analysis.py` - Pure analysis API tests (`analyze_gnn`, `summarize_hypotheses`, report-render purity/parity, discovery)

### Test Coverage
Measure on demand:

```bash
uv run --extra dev python -m pytest tests/research/ \
    --cov=src/gnn/research --cov-report=term-missing
```
