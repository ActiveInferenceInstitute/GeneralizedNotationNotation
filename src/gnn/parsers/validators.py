...
@dataclass
class GNNParserValidationResult:
...
class GNNValidator:
    ...
    def validate(self, model: GNNInternalRepresentation) -> GNNParserValidationResult:
        ...
        """
        result = GNNParserValidationResult(success=True)
    ...
    def _validate_variable(
        self, variable: Variable, result: GNNParserValidationResult
    ) -> None:
        ...
        if (
            variable.name
            and not variable.name.replace("_", "").replace("-", "").isalnum()
        ):
            result.add_issue(
                ValidationIssue(
                    severity=ValidationSeverity.WARNING,
    ...
    def _validate_variable_active_inference(
        self, variable: Variable, result: GNNParserValidationResult
    ) -> None:
        ...
        if variable.var_type in [
            VariableType.LIKELIHOOD_MATRIX,
            VariableType.TRANSITION_MATRIX,
        ]:
            if len(variable.dimensions) != 2:
                result.add_issue(
                    ValidationIssue(
                        ...
                        details={
                            "expected_dims": 2,
                            "actual_dims": len(variable.dimensions),
                        },
                    )
                )
        ...
        # Check vector variables have appropriate dimensions
            ...
            VariableType.PREFERENCE_VECTOR,
            VariableType.PRIOR_VECTOR,
        ]:
        ...
        if variable.var_type == VariableType.HIDDEN_STATE and not any(
            keyword in name_lower for keyword in ["state", "hidden", "s_"]
        ):
            result.add_issue(
                ValidationIssue(
        ...
        if variable.var_type == VariableType.ACTION and variable.data_type not in [
            DataType.BINARY,
            ...
            result.add_issue(
                ValidationIssue(
                    ...
                    component_type="variable",
                    details={
                        "current_type": variable.data_type.value,
                        "suggested_types": ["binary", "categorical", "integer"],
                    },
                )
            )
    ...
    def _validate_connections(
        ...
        for i, connection in enumerate(model.connections):
                    ...
                    result.add_issue(
                        ValidationIssue(
                            ...
                            details={
                                ...
                                "available_variables": list(variable_names),
                            },
                        )
                    )
            ...
            if connection.connection_type == ConnectionType.DIRECTED:
                common_vars = set(connection.source_variables) & set(
    ...
    def _validate_connection_active_inference(
        self,
        connection: Connection,
        model: GNNInternalRepresentation,
        result: GNNParserValidationResult,
        index: int,
    ) -> None:
                ...
                for target_var_name in connection.target_variables:
                    ...
                    if target_var and target_var.var_type == VariableType.OBSERVATION:
                        ...
                        a_matrices = [
                            ...
                            if var.var_type == VariableType.LIKELIHOOD_MATRIX
                        ]
                        if not a_matrices:
                            result.add_issue(
                                ValidationIssue(
        ...
        for source_var_name in connection.source_variables:
            ...
            if source_var and source_var.var_type == VariableType.HIDDEN_STATE:
                for target_var_name in connection.target_variables:
                    ...
                    if (
                        target_var
                        and target_var.var_type == VariableType.HIDDEN_STATE
                        and source_var_name != target_var_name
                    ):
                        # This might need B matrix
                        b_matrices = [
                            var
                            for var in model.variables
                            if var.var_type == VariableType.TRANSITION_MATRIX
                        ]
                        if not b_matrices:
                            result.add_issue(
                                ValidationIssue(
        ...
        for var_name in unreferenced:
            result.add_issue(
                ValidationIssue(
                    ...
                    component_type="variable",
                    details={
                        "suggestion": "Consider adding connections or removing if unused"
                    },
                )
            )
    ...
    def _validate_active_inference(
        self, model: GNNInternalRepresentation, result: GNNParserValidationResult
    ) -> None:
        """Validate Active Inference specific constraints."""
    ...
    def _validate_pomdp_structure(
        self,
        model: GNNInternalRepresentation,
        actinf_vars: Dict[str, bool],
        result: GNNParserValidationResult,
    ) -> None:
        ...
        has_temporal_connections = (
            any(
                connection.description and "temporal" in connection.description.lower()
                for connection in model.connections
            )
            or model.time_specification
            and model.time_specification.time_type == "Dynamic"
        )
        ...
        if has_temporal_connections and not model.time_specification:
            result.add_issue(
                ValidationIssue(
                    ...
                    component_type="active_inference",
                    details={
                        "suggestion": "Add time specification for temporal dynamics"
                    },
                )
            )
    ...
    def _validate_matrix_dimensions(
        ...
        hidden_states = [
            var for var in model.variables if var.var_type == VariableType.HIDDEN_STATE
        ]
        observations = [
            var for var in model.variables if var.var_type == VariableType.OBSERVATION
        ]
        a_matrices = [
            var
            for var in model.variables
