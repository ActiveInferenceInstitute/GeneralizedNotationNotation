...
pipeline. The executable registry is `src/gnn/mcp/mcp.py`; JSON-RPC request
handling lives in `src/gnn/mcp/server_core.py` and is re-exported through
`src/gnn/mcp/server.py`.
...
```bash
uv run --extra dev python -m pytest tests/mcp/test_mcp_configurability.py tests/mcp/test_mcp_audit.py -q
