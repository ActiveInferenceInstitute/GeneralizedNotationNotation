...
4. **Round-trip fidelity**: Reference models should round-trip for every format in the **default** `test_formats` list in `gnn/testing/round_trip_config.py` (see **[../SPEC.md](../SPEC.md)**). That list is **not** identical to all **23** `GNNFormat` values (e.g. **PNML** / **EBNF** may be excluded or shared with BNF machinery).

## Testing

Examples are used as input fixtures by:

- `gnn/testing/round_trip_tester.py` — round-trip runner for the configured formats (list in `gnn/testing/round_trip_config.py`; pytest cases in `tests/testing/test_round_trip.py`; see **[../SPEC.md](../SPEC.md)**)
- `gnn/testing/test_comprehensive.py` — comprehensive parsing validation
- `tests/gnn/test_gnn_parsing.py` — unit-level parsing tests
