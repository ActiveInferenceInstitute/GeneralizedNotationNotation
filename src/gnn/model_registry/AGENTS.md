...
```python
from gnn.model_registry.registry import ModelRegistry
...
```bash
uv run --extra dev python -m pytest tests/model_registry/ \
    --cov=src/gnn/model_registry --cov-report=term-missing
```
