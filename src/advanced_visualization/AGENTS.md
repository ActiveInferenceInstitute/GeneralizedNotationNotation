...
- `_conn_endpoints(conn_info) -> (source_variables, target_variables)` — normalizes
  the scalar `{"source", "target"}` format and the multi-variable `{"source_variables", "target_variables"}`
...
- `AdvancedVisualizer(logger=None, extractor=None)` — constructor injection
  ...
  `extract_from_content(content) -> dict` is accepted (in-memory test extractors included).
...
- `_create_network_graph` resolves real node indices via a name→index map
  over `blocks`; both extractor formats and scalar-connection entries
