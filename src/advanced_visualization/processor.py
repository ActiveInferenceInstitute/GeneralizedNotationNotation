#!/usr/bin/env python3
"""
Advanced visualization processor module.

This module contains the core advanced visualization processing logic
that is called by the thin orchestrator script (9_advanced_viz.py).
"""
...
from .data_extractor import extract_visualization_data

# Safe imports with fallback handling
try:
    ADVANCED_VISUALIZER_AVAILABLE = True
except ImportError as e:
    logging.warning(f"AdvancedVisualizer not available: {e}")
...
try:
    ...
    logging.warning(f"DashboardGenerator not available: {e}")
...
try:
    ...
    logging.warning(f"DataExtractor not available: {e}")
    ...
    try:
        ...
        with performance_tracker.track_operation("advanced_viz_processing", {"verbose": verbose, "recursive": recursive}):
            ...
            if not target_dir.exists():
                logger.warning(f"[{correlation_id}] Input directory does not exist: {target_dir}")
            ...
            if not gnn_files:
                logger.warning(f"[{correlation_id}] No GNN files found in {target_dir}")
            ...
            if total_generated > 0:
                logger.info(f"[{correlation_id}] Generated visualizations for {total_generated}/{results.total_files} files ({results.successful_visualizations} full, {results.fallback_visualizations} fallback)")
            else:
                logger.warning(f"[{correlation_id}] No visualizations generated, but processing completed")

            return True  # Always return True for pipeline continuation

    except Exception as e:
        logger.error(f"[{correlation_id}] Advanced visualization processing failed: {e}")
        if verbose:
