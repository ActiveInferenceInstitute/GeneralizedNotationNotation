...
def generate_pipeline_summary_site_mcp(output_dir: str, site_output_filename: str, verbose: bool = False) -> Dict[str, Any]:
    """
    MCP tool to generate a single HTML website summarizing all contents of the GGN pipeline output directory.
    
    This function is a wrapper around the core site generation logic to make it compatible with the MCP.
    """
    # Get the logger for the generator module to control its verbosity
    generator_logger = logging.getLogger("src.site.generator")
    if verbose:
    ...
    try:
        ...
        site_output_file_path = output_dir_path / site_output_filename
        ...
        # Ensure the parent directory for the site file exists
        site_output_file_path.parent.mkdir(parents=True, exist_ok=True)
        
        # Call the core report generation function
        generate_html_report(output_dir_path, site_output_file_path)
        
        success_msg = f"HTML site generated successfully at {site_output_file_path}"
        logger.info(success_msg)
        return {"success": True, "message": success_msg, "site_path": str(site_output_file_path)}
    except Exception as e:
        error_msg = f"An unexpected error occurred during site generation: {str(e)}"
...
def register_tools(mcp_instance):
    ...
    mcp_instance.register_tool(
        ...
        func=generate_pipeline_summary_site_mcp,
        schema=schema,
