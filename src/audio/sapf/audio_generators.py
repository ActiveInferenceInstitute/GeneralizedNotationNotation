...
class SyntheticAudioGenerator:
    ...
    def generate_from_sapf(
        ...
        try:
            ...
            if create_visualization:
                self._create_waveform_visualizations(
                    audio_data, output_file, audio_params
                )
    ...
    def _analyze_sapf_code(self, sapf_code: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            sine_match = re.search(
                r"base_freq\s+(\d+\.?\d*)\s+\+.*?sinosc\s+([\d\.]+)\s*\*", line
            )
            if sine_match:
            ...
            saw_match = re.search(
                r"base_freq\s+(\d+\.?\d*)\s+\+.*?lfsaw\s+([\d\.]+)\s*\*", line
            )
            if saw_match:
            ...
            if freq_match and not sine_match and not saw_match:
                ...
                params["oscillators"].append(
                    {"type": osc_type, "frequency": freq, "amplitude": amplitude}
                )
            ...
            if lfo_match:
                ...
                params["effects"].append(
                    {"type": "lfo", "frequency": lfo_freq, "amount": amount}
                )
            ...
            if lpf_match:
                ...
                params["filters"].append(
                    {"type": "lowpass", "cutoff": cutoff, "resonance": 0.1}
                )

            # Extract envelope information with model-specific parameters
            env_match = re.search(
                r"(\d+\.?\d*)\s+sec\s+0\s+1\s+([\d\.]+)\s+([\d\.]+)\s+env", line
            )
            if env_match:
    ...
    def _generate_audio(self, params: Dict[str, Any], duration: float) -> List[int]:
        ...
        for effect in params["effects"]:
            if effect["type"] == "lfo":
                lfo_audio = self._generate_lfo(
                    effect["frequency"], effect["amount"], samples
                )
        ...
        for filt in params["filters"]:
            if filt["type"] == "lowpass":
                audio_data = self._apply_lowpass_filter(audio_data, filt["cutoff"])

        # Apply envelope
        if params["envelopes"]:
            envelope = self._generate_envelope(params["envelopes"][0], samples)
            audio_data *= envelope
    ...
    def _generate_oscillator(
        self, frequency: float, amplitude: float, osc_type: str, samples: int
    ) -> np.ndarray:
    ...
    def _generate_lfo(
        self, frequency: float, amount: float, samples: int
    ) -> np.ndarray:
    ...
    def _create_waveform_visualizations(
        self, audio_data: List[int], output_file: Path, audio_params: Dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            fig.suptitle(
                f"Audio Analysis: {output_file.stem}", fontsize=16, fontweight="bold"
            )
            ...
            ax3.loglog(
                frequencies[1:], fft_magnitude[1:], color="#F18F01", linewidth=1.5
            )
            ...
            for osc in audio_params.get("oscillators", []):
                ...
                if freq > 0 and freq < self.sample_rate / 2:
                    ax3.axvline(
                        freq, color="red", linestyle="--", alpha=0.7, linewidth=1
                    )
            ...
            if len(audio_array) > 1024:  # Only if we have enough samples
                ...
                ax4.set_ylim(
                    0, min(2000, spec_sample_rate / 2)
                )  # Limit to 2kHz for clarity
            ...
            info_text = (
                f"Base Frequency: {audio_params.get('base_frequency', 'Unknown')} Hz\\n"
            )
...
def mix_audio_channels(
    audio_list: List[np.ndarray], weights: Optional[List[float]] = None
) -> np.ndarray:
    """
