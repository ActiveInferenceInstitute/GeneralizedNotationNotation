...
def convert_gnn_to_sapf_code(gnn_content: str, model_name: str) -> str:
    """
    Convert GNN model content to SAPF code using the processor.
    
    Args:
        gnn_content: Raw GNN file content
        model_name: Name of the model for SAPF variables
        
    Returns:
        Generated SAPF code as string
    """
    logger.debug(f"Converting GNN model '{model_name}' to SAPF code")
    
    if DEPENDENCIES_AVAILABLE:
        try:
            # Use the full SAPF processor
            return convert_gnn_to_sapf(gnn_content, model_name)
        except Exception as e:
            logger.warning(f"SAPF processor failed, using fallback: {e}")
    
    # Fallback SAPF code generation
    sapf_code_lines = [
        f"; SAPF Audio Representation for GNN Model: {model_name}",
        f"; Generated by GNN-SAPF Pipeline Step",
        "",
        "; Core schema definition",
        "{ :gnn_version \"1.4\" :sapf_version \"0.1.21\" :model_name \"" + model_name + "\" } = gnn_sapf_schema",
        "",
        "; Basic audio generation for GNN model",
        "\\generate_model_audio [",
        "  ; Base frequency (C4)",
        "  261.63 = base_freq",
        "",
        "  ; Generate primary oscillator",
        "  base_freq 0 sinosc .3 * = primary_osc",
        "",
        "  ; Add harmonic content",
        "  base_freq 2 * 0 sinosc .1 * = harmonic_osc",
        "",
        "  ; Combine oscillators", 
        "  primary_osc harmonic_osc +",
        "",
        "  ; Apply envelope",
        "  10 sec 0 1 0.8 0.2 env *",
        "] = model_audio",
        "",
        "; Execute and play",
        "generate_model_audio !",
    ...
    return "\n".join(sapf_code_lines)

def generate_synthetic_audio(sapf_code: str, output_file: Path, duration: float) -> bool:
    """
    Generate synthetic audio based on SAPF code analysis.
    ...
    Returns:
        True if synthetic audio generation succeeded
    """
    try:
        if DEPENDENCIES_AVAILABLE:
            # Use the advanced audio generator
            generator = SyntheticAudioGenerator()
            return generator.generate_from_sapf(sapf_code, output_file, duration)
        else:
            # Fallback audio generation using basic wave synthesis
            import wave
            import struct
            ...
            for i in range(samples):
                ...
                import math
                sample = 0.0
                sample += 0.3 * math.sin(2 * math.pi * base_freq * t)  # Fundamental
                sample += 0.1 * math.sin(2 * math.pi * base_freq * 2 * t)  # Second harmonic
                
                # Apply envelope
                if t < 0.1:  # Attack
                    envelope = t / 0.1
                elif t < duration - 1.0:  # Sustain
                    envelope = 1.0
                else:  # Release
                    envelope = (duration - t) / 1.0
            ...
            logger.debug(f"Fallback audio generated: {output_file}")
            return True
...
def process_single_file(
    ...
    try:
        ...
        # Convert to SAPF code
        sapf_code = convert_gnn_to_sapf_code(gnn_content, model_name)
        ...
        # Save SAPF code
        sapf_file = model_output_dir / f"{model_name}.sapf"
        with open(sapf_file, 'w') as f:
            f.write(sapf_code)
        logger.debug(f"SAPF code saved: {sapf_file}")
        
        # Generate audio file
        audio_duration = options.get('duration', 10.0)
        audio_file = model_output_dir / f"{model_name}_audio.wav"
        
        success = generate_synthetic_audio(sapf_code, audio_file, audio_duration)
        if not success:
        ...
        report_file = model_output_dir / f"{model_name}_sapf_report.json"
        with open(report_file, 'w') as f:
            json.dump(report, f, indent=2)
        
        logger.info(f"Successfully processed {model_name}: SAPF code and audio generated")
        return True
