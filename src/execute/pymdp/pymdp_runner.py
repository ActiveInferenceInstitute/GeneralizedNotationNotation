...
def execute_pymdp_script_with_outputs(
    ...
    try:
        ...
        # Delegate the subprocess envelope (run + timing + error normalization)
        # to the canonical safe executor. Function-local import: executor.py
        # imports this runner at module scope, so a module-level import would
        # be circular.
        ...
        if envelope["return_code"] == -1 and "error" in envelope:
            # The script never ran (timeout / unexpected failure). Keep the
            # pre-delegation generic-error result shape instead of writing logs.
            return {
                "success": False,
                "script_name": script_path.name,
                "error": envelope["error"],
                "traceback": f"{envelope['error_type']}: {envelope['error']}",
            }

        result_returncode = envelope["return_code"]
        result_stdout = envelope["stdout"]
        result_stderr = envelope["stderr"]
        script_success: bool = bool(envelope["success"])
        ...
        if script_success:
            ...
            metadata = extract_execution_metadata(
                script_path, result_stdout, script_output_dir
            )
            ...
            logger.error(
                f"Script execution failed with return code {result_returncode}: {script_path.name}"
            )
...
def extract_execution_metadata(
    script_path: Path, stdout_content: str, output_dir: Path
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if "A = " in stdout_content and "B = " in stdout_content:
            ...
            logger.info(
                f"Successfully extracted A and B matrices from {script_path.name}"
            )
        ...
        trace_file = generate_simulation_trace(
            stdout_content, output_dir, script_path.stem
        )
        if trace_file:
...
def generate_simulation_trace(
    stdout_content: str, output_dir: Path, script_name: str
) -> Optional[Path]:
    """
    ...
    try:
        ...
        for line in lines:
            if any(
                matrix in line for matrix in ["A = ", "B = ", "C = ", "D = ", "E = "]
            ):
...
def run_pymdp_scripts(
    ...
    """
    logger.info(
        f"Starting PyMDP script execution with output capture from: {rendered_simulators_dir}"
    )
    ...
    if execution_output_dir:
        ...
        exec_output_dir.mkdir(parents=True, exist_ok=True)
        logger.info(
            f"Using default PyMDP execution output directory: {exec_output_dir}"
        )
    ...
    logger.info(
        f"PyMDP script execution summary: {success_count} succeeded, {failure_count} failed, {total_count} total"
    )
...
def create_execution_report(
    ...
    with open(md_report_file, "w") as f:
        ...
        f.write(
            f"**Success Rate:** {report['execution_summary']['success_rate']:.1%}\n\n"
        )
        ...
        for result in execution_results:
            ...
            if result.get("success", False):
                ...
                if "metadata" in result:
                    ...
                    f.write(
                        f"- **Matrices Extracted:** {'Yes' if meta.get('matrices_found', False) else 'No'}\n"
                    )
                    f.write(
                        f"- **Agent Instantiated:** {'Yes' if meta.get('agent_instantiated', False) else 'No'}\n"
                    )
            else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Execute PyMDP scripts generated by the GNN rendering step"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
