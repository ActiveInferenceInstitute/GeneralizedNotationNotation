...
def validate_pymdp_environment() -> Dict[str, Any]:
    ...
    if detection.get("correct_package"):
        ...
        logger.info(
            f"PyMDP (inferactively-pymdp) available: version {detection.get('version', 'unknown')}"
        )
    ...
    try:
        ...
        validation_results["errors"].append(
            "NumPy not available - install with: uv pip install numpy"
        )
    ...
    try:
        ...
        validation_results["warnings"].append(
            "SciPy not available - some advanced features may be limited"
        )
    ...
    if validation_results["pymdp_available"]:
        ...
        validation_results["recommendations"].append(
            "Environment ready for PyMDP simulations"
        )
    else:
...
def get_pymdp_health_status() -> Dict[str, Any]:
    ...
    if validation_results["pymdp_available"] and len(validation_results["errors"]) == 0:
        ...
        validation_results["pymdp_available"]
