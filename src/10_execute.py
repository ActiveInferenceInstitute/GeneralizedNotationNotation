...
try:
    ...
    # Also try to import RxInfer runner if available
    try:
        from execute import rxinfer_runner
    except ImportError:
        logger.warning("RxInfer runner module not found. RxInfer execution will be disabled.")
        rxinfer_runner = None
except ImportError:
...
def main(args: argparse.Namespace) -> int:
    ...
    if pymdp_runner and hasattr(pymdp_runner, 'run_pymdp_scripts'):
        ...
        try:
            ...
            if pymdp_success:
                logger.info("  ✅ PyMDP scripts executed successfully or no scripts were found to run.")
            else:
                logger.error("  ❌ Some PyMDP scripts failed during execution.")
                overall_success = False
                
        except Exception as e:
            logger.error(f"  ❌ An unexpected error occurred during PyMDP execution step: {e}", exc_info=args.verbose)
            overall_success = False
    else:
        logger.warning("  ⚠️ PyMDP runner module or function not loaded. Skipping PyMDP execution.")
    ...
    if rxinfer_runner and hasattr(rxinfer_runner, 'run_rxinfer_scripts'):
        ...
        try:
            ...
            if rxinfer_success:
                logger.info("  ✅ RxInfer.jl scripts executed successfully or no scripts were found to run.")
            else:
                logger.error("  ❌ Some RxInfer.jl scripts failed during execution.")
                overall_success = False
                
        except Exception as e:
            logger.error(f"  ❌ An unexpected error occurred during RxInfer.jl execution step: {e}", exc_info=args.verbose)
            overall_success = False
    else:
        # Check if Julia is available before warning about RxInfer
        julia_available = check_julia_availability()
        
        if julia_available:
            logger.warning("  ⚠️ RxInfer runner module or function not loaded. Skipping RxInfer.jl execution.")
        else:
            logger.warning("  ⚠️ Julia not available in the system. Skipping RxInfer.jl execution.")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--recursive", action=argparse.BooleanOptionalAction, default=True, # Keep True if sensible default for standalone execution of this step
                        help="Recursively search for scripts in their base output folder.")
    ...
    parser.add_argument("--pymdp-only", action="store_true",
    ...
    # If verbose, also set the runner loggers to DEBUG for more detailed output
    if parsed_args.verbose:
        # Set PyMDP runner logger to DEBUG if available
        ...
        # Set RxInfer runner logger to DEBUG if available
        if rxinfer_runner and hasattr(rxinfer_runner, '__name__'):
            logging.getLogger(rxinfer_runner.__name__).setLevel(logging.DEBUG)
            logging.getLogger(__name__).debug(f"Verbose logging extended to '{rxinfer_runner.__name__}' module.")

    # Quieten other noisy libraries if necessary
    logging.getLogger('matplotlib').setLevel(logging.WARNING)
    
    # Modify runner modules based on command-line flags
    if parsed_args.pymdp_only and pymdp_runner:
        rxinfer_runner = None
        logger.info("Only executing PyMDP scripts (--pymdp-only flag set)")
    elif parsed_args.rxinfer_only and rxinfer_runner:
        pymdp_runner = None
        logger.info("Only executing RxInfer.jl scripts (--rxinfer-only flag set)")
