...
def execute_pipeline_step(script_name: str, args: PipelineArguments, logger) -> Dict[str, Any]:
    ...
    try:
        ...
        # Ensure numbered output directories exist for consistency
        try:
            project_root = Path(__file__).parent.parent
            ...
            def _safe_symlink(src: Path, dst: Path) -> None:
                try:
                    # If destination exists (file, dir, or link), remove it to replace
                    ...
                    # Fallback: create destination directory and a marker pointing to source
                    dst.mkdir(parents=True, exist_ok=True)
                    marker = dst / ".source_path.txt"
                    try:
                        marker.write_text(str(src))
                    except Exception:
                        pass

            # Mapping from script to actions required to enforce numbered outputs
            if step_result["status"].startswith("SUCCESS"):
                if script_name == "1_setup.py":
                    _safe_symlink(output_root / "setup_artifacts", output_root / "1_setup_output")

                elif script_name == "2_tests.py":
                    numbered_dir = output_root / "2_tests_output"
                    numbered_dir.mkdir(parents=True, exist_ok=True)
                    # Symlink subdirectories if present
                    tr = output_root / "test_results"
                    if tr.exists():
                        _safe_symlink(tr, numbered_dir / "test_results")
                    ta = output_root / "test_artifacts"
                    if ta.exists():
                        _safe_symlink(ta, numbered_dir / "test_artifacts")

                elif script_name == "5_type_checker.py":
                    numbered_dir = output_root / "5_type_checker_output"
                    numbered_dir.mkdir(parents=True, exist_ok=True)
                    for fname in ("type_check_results.json", "type_check_summary.json", "global_type_analysis.json"):
                        src_file = output_root / fname
                        if src_file.exists():
                            try:
                                shutil.copy2(src_file, numbered_dir / fname)
                            except Exception:
                                pass

                elif script_name == "7_export.py":
                    _safe_symlink(output_root / "gnn_exports", output_root / "7_export_output")

                elif script_name == "8_visualization.py":
                    _safe_symlink(output_root / "visualization", output_root / "8_visualization_output")

                elif script_name == "12_execute.py":
                    _safe_symlink(output_root / "execution_results", output_root / "12_execute_output")

                elif script_name == "15_audio.py":
                    _safe_symlink(output_root / "audio", output_root / "15_audio_output")

                elif script_name == "20_website.py":
                    # For website outputs, avoid symlinks to prevent VCS issues traversing links
                    website_src = output_root / "website"
                    website_dst_root = output_root / "20_website_output"
                    try:
                        if website_dst_root.exists() and website_dst_root.is_symlink():
                            website_dst_root.unlink()
                        website_dst_root.mkdir(parents=True, exist_ok=True)
                        website_dst = website_dst_root / "website"
                        if website_dst.exists() and website_dst.is_dir():
                            # Remove existing to ensure a clean copy
                            shutil.rmtree(website_dst)
                        if website_src.exists():
                            shutil.copytree(website_src, website_dst)
                    except Exception:
                        pass

        except Exception:
            # Never fail the step due to post-processing of directories
            pass
