...
"""
    ...
    @pytest.fixture
    def pipeline_directories(
        self, safe_filesystem: Any, sample_gnn_content: str
    ) -> Dict[str, Any]:
        ...
        gnn_file = safe_filesystem.create_file(
            "input/gnn_files/test_pipeline_agent.md", sample_gnn_content
        )
    ...
    @pytest.mark.slow
    def test_render_execute_analyze_flow(
        self, pipeline_directories: Dict[str, Any]
    ) -> None:
        ...
        # Step 1: Render
        render_result = process_render(
            dirs["input_dir"], dirs["base_output"], verbose=True
        )
...
class TestExecuteAnalyzeIntegration:
    ...
    @pytest.mark.integration
    def test_execution_results_analyzable(
        self, simulated_execute_output: Any, safe_filesystem: Any
    ) -> None:
...
class TestRenderExecuteIntegration:
    ...
    @pytest.mark.integration
    @pytest.mark.slow
    def test_render_output_executable(
        self, safe_filesystem: Any, sample_gnn_spec: Dict[str, Any]
    ) -> None:
        """Test that render output can be executed."""
        # Create pipeline structure
        render_output = safe_filesystem.create_dir(
        ...
        try:
            ...
            ok, msg, artifacts = render_gnn_spec(
                sample_gnn_spec, "pymdp", render_output
            )
