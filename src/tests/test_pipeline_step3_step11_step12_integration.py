...
GNN_FILE_PATH = Path(__file__).parent.parent.parent / "input" / "gnn_files" / "basics" / "static_perception.md"
...
class TestStep3ParseProducesModel:
    ...
    def test_parse_file_returns_parse_result_with_model(self):
        ...
        try:
            result = parser.parse_file(tmp_path)
            # ParseResult should have a model field
    ...
    def test_schema_parse_connections_from_gnn_content(self):
        ...
        assert len(edges) >= 2  # A>s, B>s, s-o
...
class TestStep11RenderConsumesParseOutput:
    ...
    def _minimal_spec(self):
        return {
            ...
            "state_space": {
                ...
                "s": {"dimensions": [2, 1], "type": "float"},
                "o": {"dimensions": [2, 1], "type": "int"},
            },
            "connections": [
                {"source": "A", "target": "s", "directed": True},
                {"source": "s", "target": "o", "directed": False},
            ],
            "parameters": {
                "A": "{(0.9,0.1),(0.1,0.9)}",
            },
    ...
    def test_jax_render_produces_output_path(self, tmp_path):
        ...
        # If rendering is supported, file should exist
        if success:
...
class TestStep12ExecutionHandshake:
    ...
    def test_pymdp_executor_importable(self):
        """PyMDP executor is importable (doesn't require PyMDP installed)."""
        try:
            from execute.pymdp import processor as pymdp_proc
            assert pymdp_proc is not None
        except ImportError:
            pytest.skip("execute.pymdp not available")
    ...
    def test_executor_with_missing_file_returns_failure(self):
        ...
        # Must not raise; should return a failure dict/object
        assert result is not None
