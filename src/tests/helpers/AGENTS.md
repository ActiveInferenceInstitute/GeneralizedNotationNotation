...
canonical sample content, an MCP registry test adapter, sample-model path helpers,
...
├── mcp_stubs.py           # MCPTools: in-memory MCP registry test adapter
...
- `MCPTools` — in-memory MCP registry test adapter (`register_tool` / `register_resource` / `execute_tool`); the conftest `test_mcp_tools` fixture returns an instance
