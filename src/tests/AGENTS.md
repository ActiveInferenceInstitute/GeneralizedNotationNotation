...
| `MCPTools` | `mcp_stubs.py` | In-memory MCP registry test adapter (`register_tool` / `register_resource` / `execute_tool`). The `test_mcp_tools` fixture returns an instance; module wiring tests should adopt it instead of building local in-memory registries. |
