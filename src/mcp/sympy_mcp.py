...
async def get_sympy_integration() -> Any:
    ...
    if sympy_integration_instance is None:
        try:
            ...
            sympy_integration_instance = await create_sympy_integration(
                server_executable
            )
...
async def validate_equation_tool(
    equation: str, context: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
...
async def validate_matrix_tool(
    matrix_data: List[List[Any]], matrix_type: str = "transition"
) -> Dict[str, Any]:
    """Tool function for matrix validation"""
    try:
        integration = await get_sympy_integration()
        result = await integration.validate_matrix_stochasticity(
...
async def analyze_stability_tool(
    transition_matrices: List[List[List[Any]]],
) -> Dict[str, Any]:
...
async def solve_equation_tool(
    equation: str, variable: str, domain: str = "COMPLEX"
) -> Dict[str, Any]:
    ...
    try:
        ...
        solution = await integration.sympy_client.solve_algebraically(
            expr_key, variable, domain
        )
...
async def initialize_sympy_tool(
...
def validate_equation_tool_sync(
    equation: str, context: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
...
def analyze_stability_tool_sync(
    transition_matrices: List[List[List[Any]]],
) -> Dict[str, Any]:
...
def initialize_sympy_tool_sync(
    server_executable: Optional[str] = None,
) -> Dict[str, Any]:
