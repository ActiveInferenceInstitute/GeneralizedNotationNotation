...
Expected outputs:
  - LLM analysis results in the specified output directory
...
If you encounter errors:
  - Check that LLM dependencies are installed (openai, anthropic, etc.)
  - Check that src/llm/ contains LLM modules
  - Check that the output directory is writable
  - Verify API keys are configured if using external LLM services
"""
...
def process_llm_standardized(
    ...
    **kwargs
    ...
    try:
        ...
        logger.info(f"Found {len(gnn_files)} GNN files to process with LLM")
        
        # Process files using the LLM module
        success = process_llm(
        ...
        if success:
            log_step_success(logger, f"Successfully processed {len(gnn_files)} GNN models with LLM")
        else:
...
def main():
    ...
    try:
        ...
        success = process_llm_standardized(
