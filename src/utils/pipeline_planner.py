...
def generate_execution_plan(
    ...
    execution_plan = {
        ...
        "high_risk_count": 0,
        "expected_warnings": 0,
        "resource_requirements": {"peak_memory_mb": 0, "disk_space_mb": 0},
    }
    ...
    for script_name, description in steps_to_execute:
        ...
        if script_name == "12_execute.py":
            if "11_render.py" not in [s[0] for s in steps_to_execute]:
                step_plan["warnings"].append(
                    "Render step not included - execution may degrade gracefully"
                )

        if script_name == "8_visualization.py":
            step_plan["warnings"].append(
                "May encounter matplotlib backend issues in headless environments"
            )
        ...
        if script_name == "15_audio.py":
            step_plan["warnings"].append(
                "Audio libraries may not be available in all environments"
            )

        # Update counters and resource estimates
        execution_plan["estimated_duration_minutes"] += (
            step_plan["estimated_duration_seconds"] / 60
        )
        execution_plan["resource_requirements"]["peak_memory_mb"] += step_plan[
            "resource_impact"
        ]["memory_mb"]
        execution_plan["resource_requirements"]["disk_space_mb"] += step_plan[
            "resource_impact"
        ]["disk_mb"]
...
def estimate_pipeline_resources(steps_to_execute: List[tuple]) -> Dict[str, Any]:
    ...
    return {
        ...
        "memory_intensive_steps": len(
            [s for s in script_names if s in memory_intensive_steps]
        ),
        "disk_intensive_steps": len(
            [s for s in script_names if s in disk_intensive_steps]
        ),
    }
...
def generate_risk_assessment(steps_to_execute: List[tuple]) -> Dict[str, Any]:
    ...
    if "12_execute.py" in script_names and "11_render.py" not in script_names:
        risk_factors["dependency_risks"].append(
            "Execute step without render step - high failure risk"
        )
        risk_score += 20

    if (
        any(
            step in script_names for step in ["8_visualization.py", "9_advanced_viz.py"]
        )
        and "3_gnn.py" not in script_names
    ):
        risk_factors["dependency_risks"].append(
            "Visualization steps without GNN parsing - will fail"
        )
        risk_score += 30

    # External service risks
    if "13_llm.py" in script_names:
        risk_factors["external_service_risks"].append(
            "LLM step depends on external API availability"
        )
        risk_score += 15
    ...
    if len([s for s in script_names if s in memory_intensive]) > 2:
        risk_factors["resource_risks"].append(
            "Multiple memory-intensive steps may cause resource exhaustion"
        )
        risk_score += 10

    # Compatibility risks
    if "8_visualization.py" in script_names:
        risk_factors["compatibility_risks"].append(
            "Matplotlib backend compatibility issues in headless environments"
        )
        risk_score += 10

    if "15_audio.py" in script_names:
        risk_factors["compatibility_risks"].append(
            "Audio library compatibility varies across systems"
        )
        risk_score += 5
...
def create_execution_timeline(steps_to_execute: List[tuple]) -> List[Dict[str, Any]]:
    ...
    for script_name, description in steps_to_execute:
        ...
        can_start_immediately = all(
            dep.replace(".py", "") in [s.replace(".py", "") for s in completed_steps]
        ...
        for other_script, _ in steps_to_execute:
            if (
                other_script != script_name
                and other_script not in step_deps
                and script_name not in dependencies.get(other_script, [])
            ):
