...
def get_venv_python(script_dir: Path) -> tuple[Path | None, Path | None]:
        ...
        if venv_path.is_dir():
            ...
            for py_exec in potential_python_executables:
                if py_exec.exists() and py_exec.is_file():
                    ...
                    logger.debug(
                        f"🐍 Found virtual environment Python: {venv_python_path}"
                    )
                    break
                ...
                for python_version_dir in lib_path.iterdir():
                    if (
                        ...
                        if current_site_packages.is_dir():
                            ...
                            logger.debug(
                                f"📦 Found site-packages at: {site_packages_path}"
                            )
                            break
    ...
    if not venv_python_path:
        logger.warning(
            "⚠️ Virtual environment Python not found. Using system Python. This may lead to issues if dependencies are not globally available."
        )
