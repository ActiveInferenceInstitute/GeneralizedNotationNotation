...
class DependencyManager:
    ...
    def _setup_dependency_groups(self) -> Any:
        ...
        core = DependencyGroup(
            "core", "Core Python dependencies required for basic operation"
        )
        core.add_dependency(
            DependencyInfo(
                "numpy",
                min_version="1.19.0",
                install_command="uv add numpy",
        ...
        core.add_dependency(
            DependencyInfo(
                "pathlib", required=False, description="Path handling (built-in)"
            )
        )
        ...
        viz = DependencyGroup(
            "visualization", "Visualization and plotting dependencies", optional=True
        )
        viz.add_dependency(
            DependencyInfo(
                "matplotlib",
                min_version="3.3.0",
                install_command="uv add matplotlib",
        ...
        viz.add_dependency(
            DependencyInfo(
                "seaborn",
                min_version="0.11.0",
                install_command="uv add seaborn",
                fallback_available=True,
        ...
        viz.add_dependency(
            DependencyInfo(
                "networkx",
                min_version="2.5",
                install_command="uv add networkx",
                fallback_available=True,
        ...
        sim = DependencyGroup(
            "simulation", "Simulation engine dependencies", optional=True
        )
        sim.add_dependency(
            DependencyInfo(
                "pymdp",
                min_version="0.0.8",
                install_command="uv add pymdp",
                fallback_available=True,
                description="Active Inference simulations",
            )
        )
        sim.add_dependency(
            DependencyInfo(
                "jax",
                min_version="0.3.0",
                install_command="uv add jax",
                fallback_available=True,
                description="JAX-based computations",
            )
        )
        sim.add_dependency(
            DependencyInfo(
        ...
        # Audio Dependencies
        audio = DependencyGroup(
            "audio", "Audio processing and generation dependencies", optional=True
        )
        audio.add_dependency(
            DependencyInfo(
                "librosa",
                min_version="0.8.0",
                install_command="uv add librosa",
                fallback_available=True,
        ...
        audio.add_dependency(
            DependencyInfo(
                "soundfile",
                min_version="0.10.0",
                install_command="uv add soundfile",
                fallback_available=True,
        ...
        audio.add_dependency(
            DependencyInfo(
                "pedalboard",
                min_version="0.5.0",
                install_command="uv add pedalboard",
                fallback_available=True,
        ...
        # LLM Dependencies
        llm = DependencyGroup(
            "llm", "Large Language Model integration dependencies", optional=True
        )
        llm.add_dependency(
            DependencyInfo(
                "openai",
                min_version="1.0.0",
                install_command="uv add openai",
                fallback_available=True,
        ...
        llm.add_dependency(
            DependencyInfo(
                "anthropic",
                min_version="0.3.0",
                install_command="uv add anthropic",
                fallback_available=True,
        ...
        llm.add_dependency(
            DependencyInfo(
                "tiktoken",
                install_command="uv add tiktoken",
                fallback_available=True,
        ...
        ml = DependencyGroup(
            "machine_learning",
            "Machine learning and statistics dependencies",
            optional=True,
        )
        ml.add_dependency(
            DependencyInfo(
                "scikit-learn",
                min_version="0.24.0",
                install_command="uv add scikit-learn",
                fallback_available=True,
        ...
        dev = DependencyGroup(
            "development", "Development and testing dependencies", optional=True
        )
        dev.add_dependency(
            DependencyInfo(
                "pytest",
        ...
        dev.add_dependency(
            DependencyInfo(
                "black",
                install_command="uv add --dev black",
                fallback_available=False,
        ...
        dev.add_dependency(
            DependencyInfo(
                "flake8",
                install_command="uv add --dev flake8",
                fallback_available=False,
        ...
        external = DependencyGroup(
            "external", "External system dependencies", optional=True
        )
        external.add_dependency(
            DependencyInfo(
                "julia",
                required=False,
                fallback_available=True,
    ...
    def check_all_dependencies(self) -> Dict[str, Any]:
        ...
        for group_name in self.dependency_groups:
            ...
            all_results["overall_summary"]["total_dependencies"] += summary.get(
                "total", 0
            )
            all_results["overall_summary"]["available_dependencies"] += summary.get(
                "available", 0
            )
            all_results["overall_summary"]["missing_dependencies"] += summary.get(
                "missing", 0
            )
            ...
            if group.optional:
                ...
                required_available = sum(
                    1
                    for dep_result in group_results["dependencies"]
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        missing_core = [
            dep
            for dep in core_results.get("dependencies", [])
            if dep["required"] and not dep["available"]
        ]

        if missing_core:
            install_commands = [
                dep["install_command"] for dep in missing_core if dep["install_command"]
            ]
            if install_commands:
                recommendations.append(
                    f"Install missing core dependencies: {'; '.join(install_commands)}"
                )

        # Check for visualization dependencies
        viz_results = results["groups"].get("visualization", {})
        missing_viz = [
            dep for dep in viz_results.get("dependencies", []) if not dep["available"]
        ]
        if missing_viz and len(missing_viz) == len(viz_results.get("dependencies", [])):
            recommendations.append(
                "Consider installing visualization dependencies for chart generation: uv add matplotlib seaborn plotly"
            )

        # Check for simulation dependencies
        sim_results = results["groups"].get("simulation", {})
        missing_sim = [
            dep for dep in sim_results.get("dependencies", []) if not dep["available"]
        ]
        if missing_sim and len(missing_sim) == len(sim_results.get("dependencies", [])):
            recommendations.append(
                "Consider installing simulation dependencies for PyMDP/JAX execution: uv add pymdp jax flax"
            )
        ...
        julia_available = any(
            dep["available"]
            for dep in external_results.get("dependencies", [])
            if dep["name"] == "julia"
        )
        if not julia_available:
            recommendations.append(
                "Consider installing Julia for RxInfer.jl simulations: https://julialang.org/downloads/"
            )

        # Check for LLM dependencies
        llm_results = results["groups"].get("llm", {})
        missing_llm = [
            dep for dep in llm_results.get("dependencies", []) if not dep["available"]
        ]
        if missing_llm and len(missing_llm) == len(llm_results.get("dependencies", [])):
            recommendations.append(
                "Consider installing LLM dependencies for AI analysis: uv add openai anthropic"
            )
    ...
    def check_step_dependencies(self, step_name: str) -> Dict[str, Any]:
                ...
                if dep_result["required"] and not dep_result["available"]:
                    if not group.optional:
                        critical_missing.append(
                            f"{dep_result['name']}: {dep_result['message']}"
                        )
                    else:
                        warnings.append(
                            f"{dep_result['name']}: {dep_result['message']}"
                    ...
                    not dep_result["available"] and not dep_result["fallback_available"]
                ):
                    warnings.append(
                        f"Optional {dep_result['name']}: {dep_result['message']}"
                    )
        ...
        # Generate recommendations
        if critical_missing:
            step_results["recommendations"].append(
                f"Install critical dependencies for {step_name}"
            )
        if warnings:
            step_results["recommendations"].append(
                f"Consider installing optional dependencies for full {step_name} functionality"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Groups: {results['overall_summary']['healthy_groups']} healthy, "
        f"{results['overall_summary']['degraded_groups']} degraded, "
        f"{results['overall_summary']['failed_groups']} failed"
    )
    print(
        f"Dependencies: {results['overall_summary']['available_dependencies']}/"
