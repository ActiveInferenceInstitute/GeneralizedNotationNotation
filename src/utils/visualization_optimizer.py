...
class VisualizationCache:
    ...
    def cache_visualization(self, cache_key: str, files: List[str]) -> Any:
        ...
        self.metadata[cache_key] = {
            ...
            "files": files,
            "size_mb": sum(Path(f).stat().st_size for f in files if Path(f).exists())
            / (1024 * 1024),
        }
        self._save_metadata()
    ...
    def _cleanup_old_cache(self) -> Any:
        ...
        entries = [
            (key, datetime.fromisoformat(entry["timestamp"]))
...
class DataSampler:
    ...
    def should_sample(self, data: Dict[str, Any]) -> bool:
        ...
        return (
            ...
            or edge_count > self.max_edges
            or matrix_size > self.max_matrix_size
        )
    ...
    def sample_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "edges" in data and "_sampling_applied" in sampled_data:
            ...
            sampled_edges = [
                ...
                if edge.get("source") in sampled_node_ids
                and edge.get("target") in sampled_node_ids
            ]
        ...
        **kwargs: Any,
        ...
        if len(tasks) == 1:
            ...
            try:
                ...
                results["failed_files"].append(
                    {"file": str(tasks[0]["file_path"]), "error": str(e)}
                )
        else:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_task = {
                    executor.submit(
                        self._safe_process_task, processing_func, task
                    ): task
                    for task in tasks
                }
                ...
                for future in as_completed(future_to_task):
                    ...
                    try:
                        ...
                        results["failed_files"].append(
                            {"file": str(task["file_path"]), "error": str(e)}
                        )
        ...
        if len(files) > 1:
            ...
            results["parallel_efficiency"] = min(
                1.0, sequential_estimate / (total_time * self.max_workers)
            )
        else:
...
class VisualizationOptimizer:
    ...
    def optimize_single_file_processing(
        self,
        content: str,
        model_name: str,
        output_dir: Path,
        ...
        try:
            ...
            self.logger.debug(
                f"Generated {len(result_files)} visualizations for {model_name} in {processing_time:.2f}s"
            )
    ...
    def optimize_batch_processing(
        ...
        **kwargs: Any,
        ...
        self.logger.info(
            f"Processing {len(files)} files with optimization (parallel={self.parallel_processor.max_workers} workers)"
        )
...
def optimize_visualization_processing(
    ...
    return optimizer.optimize_batch_processing(
        files, output_dir, processing_func, **kwargs
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Visualization optimizer initialized with {optimizer.parallel_processor.max_workers} workers"
    )
