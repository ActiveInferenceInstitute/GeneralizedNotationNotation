...
def batch_write_files(
    files_data: List[Dict[str, Any]], output_dir: Path
) -> Dict[str, Any]:
    """
    ...
    for file_data in files_data:
        ...
        try:
            ...
            if isinstance(content, str):
                ...
                with tempfile.NamedTemporaryFile(
                    mode="wb", dir=file_path.parent, delete=False, suffix=".tmp"
                ) as tmp:
                    tmp.write(content)
                    tmp_name = tmp.name
                os.replace(tmp_name, file_path)
    ...
    return {
        ...
        "throughput_mbps": (total_size / (1024 * 1024)) / (end_time - start_time)
...
def get_file_performance_metrics(file_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "read_throughput_mbps": (stat.st_size / (1024 * 1024)) / read_time
            if read_time > 0
            else 0,
...
def create_temp_file_with_content(
    content: Union[str, bytes], suffix: str = ".tmp"
) -> Path:
    """
    ...
    """
    with tempfile.NamedTemporaryFile(
        mode="w" if isinstance(content, str) else "wb", suffix=suffix, delete=False
    ) as f:
        if isinstance(content, str):
            f.write(content)
        else:
    ...
    for temp_file in temp_files:
        try:
            if temp_file.exists():
                ...
                temp_file.unlink()
                results.append(
                    {"path": str(temp_file), "success": True, "size_bytes": size}
                )
            else:
                results.append(
                    {"path": str(temp_file), "success": True, "size_bytes": 0}
                )
        except Exception as e:
            results.append({"path": str(temp_file), "success": False, "error": str(e)})
