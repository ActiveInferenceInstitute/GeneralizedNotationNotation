...
-   **What:** This step has two primary functions:
    ...
    2.  **Generate Summary Reports:**
        *   Creates an export-step-specific summary (`5_export_step_report.md`) detailing export activities.
        *   Generates a basic overall file listing (`gnn_processing_summary.md`) of GNN files found in the target directory. For a comprehensive execution summary of all pipeline steps, refer to the `pipeline_execution_summary.json` file generated by `main.py` in the root output directory.
-   **Why:**
    ...
    2.  To provide reports related to the export process.
-   **How:**
    ...
    -   For summary report generation:
        ...
        -   The `gnn_processing_summary.md` currently lists GNN files found for processing.
-   **Output:**
...
-   **How:**
    ...
    -   The `pymdp_runner.py` script:
        -   Locates Python scripts (`*.py`) within `<pipeline_output_dir>/gnn_rendered_simulators/pymdp/`.
...
-   **Output:** Console logs detailing the execution status of each simulator script. Any files created by the executed simulators themselves will be in their respective current working directories (typically where the simulator script itself resides).
...
-   **`src/render/`**: Contains the core GNN rendering logic (e.g., in `render.py`) used by `9_render.py` to convert GNN specifications into executable simulators.
