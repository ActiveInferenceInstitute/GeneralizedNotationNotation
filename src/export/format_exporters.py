...
def _parse_matrix_string(matrix_str: str) -> Any:
    ...
    if not processed_str:
        logger.debug(
            f"Matrix string was empty after comment stripping (original: '{matrix_str}')"
        )
    ...
    try:
        ...
        if (
            ...
            and len(parsed_value) == 1
            and isinstance(parsed_value[0], (list, tuple))
            and processed_str.startswith("[(")
            and processed_str.endswith(")]")
        ):
            if (
                processed_str.count("(") == 1 and processed_str.count(")") == 1
            ):  # Check if it was a single tuple in original like {(...)}
                parsed_value = list(parsed_value[0])  # Convert the inner tuple to list

        logger.debug(
            f"Parsed matrix string (original: '{matrix_str}') to (processed for eval: '{processed_str}'): {parsed_value}"
        )
        return parsed_value
    except Exception as e:
        logger.warning(
            f"Could not parse matrix string with ast.literal_eval: '{processed_str}'. Error: {e}. Returning as raw string."
        )
        return matrix_str
...
def _parse_initial_parameterization_section(section_content: str) -> Dict[str, Any]:
    ...
    for line in section_content.split("\n"):
        ...
        if match:
            if current_key:
                data[current_key] = _parse_matrix_string("\n".join(current_value_lines))
...
def _gnn_model_to_dict(gnn_file_path_str: str) -> Dict[str, Any]:
    ...
    section_regex = (
        r"^##\s*([A-Za-z0-9_\s]+?)\s*$(.*?)(?=^##\s*[A-Za-z0-9_\s]+?\s*$|\Z)"
    )
    for match in re.finditer(section_regex, content, re.MULTILINE | re.DOTALL):
        ...
        if parser:
            model[name.lower()] = parser(raw_content)
        elif name == "ModelName":
            model["name"] = raw_content
    return model
...
def export_to_json_gnn(
    gnn_model: Dict[str, Any], output_file_path: Union[str, Path]
) -> tuple[bool, str]:
...
def export_to_xml_gnn(
    ...
    try:
        ...
        if (
            not safe_root_tag
            or not safe_root_tag[0].isalpha()
            and safe_root_tag[0] != "_"
        ):
...
def export_to_json_adjacency_list(
    gnn_model: Dict[str, Any], output_file_path: str
) -> Tuple[bool, str]:
    ...
    try:
        ...
        logger.debug(
            f"Successfully exported to JSON Adjacency List: {output_file_path}"
        )
