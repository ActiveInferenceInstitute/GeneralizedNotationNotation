...
object ActiveInferenceNeuralResponseModelv1Model {
  ...
  // Morphisms
  val AToB: A => B = identity
  val CToG: C => G = identity
  val C_modToB: C_mod => B = identity
  val C_modToM_alloc: C_mod => M_alloc = identity
  val DToV_m: D => V_m = identity
  val EToP: E => P = identity
  val GToP: G => P = identity
  val HToB: H => B = identity
  val MToB: M => B = identity
  val M_allocToB: M_alloc => B = identity
  val PToB: P => B = identity
  val PToC_mod: P => C_mod = identity
  val V_mToA: V_m => A = identity
  val V_mToA: V_m => A = identity
  val V_mToB: V_m => B = identity
  val WToA: W => A = identity
  val WToB: W => B = identity
