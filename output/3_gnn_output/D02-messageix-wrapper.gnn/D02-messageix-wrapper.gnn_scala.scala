...
object MESSAGEixNationalWrapperBlanketModel {
  ...
  // Morphisms
  val KTop: K => p = identity
  val capTos: cap => s = identity
  val dTos: d => s = identity
  val pTod: p => d = identity
  val sToJ: s => J = identity
  val sToK: s => K = identity
  val sTox: s => x = identity
  val xToe: x => e = identity
  val xTop: x => p = identity
