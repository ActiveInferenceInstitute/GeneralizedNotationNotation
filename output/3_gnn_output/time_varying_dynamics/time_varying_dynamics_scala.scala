...
object TimeVaryingTransitionDynamicsAgentModel {
  ...
  // Morphisms
  val AToo_t: A => o_t = identity
  val B_tTos_t+1: B_t => s_t+1 = identity
  val CToo_t: C => o_t = identity
  val DTos_t: D => s_t = identity
  val s_tToA: s_t => A = identity
  val s_tToB_t: s_t => B_t = identity
  val u_tToB_t: u_t => B_t = identity
