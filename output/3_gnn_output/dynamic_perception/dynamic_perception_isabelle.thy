theory DynamicPerceptionModel
imports Main
begin

text \<open>Dynamic Perception Model\<close>
text \<open>A dynamic perception model extending the static model with temporal dynamics:
...
- Suitable for tracking hidden sources from noisy observations\<close>
