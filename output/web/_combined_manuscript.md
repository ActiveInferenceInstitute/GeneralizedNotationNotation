...
This section reports what the project has actually produced and how each quantitative claim is grounded. Every number below is substituted at render time from the deterministic producer, which reads the repository state at commit 360a666d0, so each figure here is regenerated from the artifacts it describes rather than transcribed.
...
The repository's scale is itself evidence of the surface that the gates and pipeline cover, and it is reported in @fig:repo_metrics directly from the tracked files at commit 360a666d0.
...
The test suite comprises 452 test files containing 4829 test functions, exercising a source base of 672 Python files across 44 packages (197019 lines of source). The Model Context Protocol surface — which exposes GNN's capabilities to external agents and tools — provides 141 tools across 32 modules. The pipeline itself runs as 25 steps (0–24), and 6 figure artifacts from the rendering of figures, models, and reports are committed under `output/`, of which 6 are the manuscript's own.
...
Because the variables file is regenerated before rendering, the counts in the rendered PDF track the repository state at the commit recorded in `output/data/manuscript_variables.json` (360a666d0): a code change that alters, for example, the test inventory (452 test files, 4829 test functions) propagates into the prose on the next regeneration without any manual editing.
