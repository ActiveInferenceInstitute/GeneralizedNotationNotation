#!/usr/bin/env python3
"""
PyMDP Agent Script - Multifactor_PyMDP_Agent_v1
...
# INFO: Found StateSpaceBlock data with 20 items.
# INFO: Parsed ModelParameters from raw text: {'num_hidden_states_factors': [2, 3], 'num_obs_modalities': [3, 3, 3], 'num_control_factors': '[1, 3]   # B_f0 actions_dim=1 (uncontrolled), B_f1 actions_dim=3 (controlled by pi_f1)'}
# INFO: Extracted state factors from ModelParameters: [2, 3]
# INFO: Extracted observation modalities from ModelParameters: [3, 3, 3]
...
A_modality_0 = utils.norm_dist(np.ones((3, 2, 3)))
A_modality_1 = utils.norm_dist(np.ones((3, 2, 3)))
A_modality_2 = utils.norm_dist(np.ones((3, 2, 3)))
A = np.empty(3, dtype=object)
A[0] = A_modality_0
A[1] = A_modality_1
A[2] = A_modality_2
B_factor_0 = utils.norm_dist(np.eye(2)[:, :, np.newaxis])
B_factor_1 = utils.norm_dist(np.eye(3)[:, :, np.newaxis])
B = np.empty(2, dtype=object)
B[0] = B_factor_0
B[1] = B_factor_1
C_modality_0 = np.zeros(3)
C_modality_1 = np.zeros(3)
C_modality_2 = np.zeros(3)
C = np.empty(3, dtype=object)
C[0] = C_modality_0
C[1] = C_modality_1
C[2] = C_modality_2
D_factor_0 = np.ones(2) / 2.0
D_factor_1 = np.ones(3) / 3.0
...
if __name__ == "__main__":
    ...
    if __name__ == '__main__':
        ...
        # agent = Multifactor_PyMDP_Agent_v1  # Agent is already instantiated above as 'agent'
        ...
        obs_names = [f'Modality_{i}' for i in range(3)] if 3 > 0 else []
        state_names = [f'Factor_{i}' for i in range(2)] if 2 > 0 else []
        num_modalities = 3
        num_factors = 2
        control_fac_idx = []
