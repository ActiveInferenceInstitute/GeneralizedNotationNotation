...
# DEBUG: Successfully parsed stringified direct_num_obs: [3, 3]
# DEBUG: Successfully parsed stringified direct_num_states: [4, 3]
# INFO: Observation dimensions (num_obs) derived directly from gnn_spec.num_obs_modalities: [3, 3]
# INFO: Observation names generated as defaults (gnn_spec.obs_modality_names not found or mismatched).
# INFO: Hidden state dimensions (num_states) derived directly from gnn_spec.num_hidden_states_factors: [4, 3]
...
# --- GNN Model: Airplane_Trading_POMDP_v10 ---
...
# --- Matrix Definitions ---
A = utils.obj_array(2)
A[0] = utils.norm_dist(np.ones((3, 4, 3))) # Defaulted to uniform
A[1] = utils.norm_dist(np.ones((3, 4, 3))) # Defaulted to uniform
B = utils.obj_array(num_factors)
B[0] = np.eye(4)[:, :, np.newaxis] # Default for uncontrolled (identity)
B[1] = np.eye(3)[:, :, np.newaxis] # Default for uncontrolled (identity)
C = utils.obj_array_zeros(num_obs)
D = utils.obj_array(2)
D[0] = utils.norm_dist(np.ones(4)) # Default: uniform D for factor 0
D[1] = utils.norm_dist(np.ones(3)) # Default: uniform D for factor 1
E = None

# --- Agent Instantiation ---
Airplane_Trading_POMDP_v10 = Agent(
    A=A,
    B=B,
    C=C,
    D=D
...
if __name__ == '__main__':
    ...
    agent = Airplane_Trading_POMDP_v10
    print(f"Agent 'Airplane_Trading_POMDP_v10' initialized with {agent.num_factors if hasattr(agent, 'num_factors') else 'N/A'} factors and {agent.num_modalities if hasattr(agent, 'num_modalities') else 'N/A'} modalities.")
