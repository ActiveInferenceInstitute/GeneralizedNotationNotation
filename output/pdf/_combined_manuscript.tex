...
generated from the producer's token map at commit ba73789bf. A
...
state at commit ba73789bf, so each figure here is regenerated from the
...
ba73789bf.}\label{fig:family_matrix}
\end{figure}
...
ba73789bf --- the same token map that substitutes the prose counts, so
...
\breaktt{output/data/manuscript\_variables.json} (ba73789bf): a code
