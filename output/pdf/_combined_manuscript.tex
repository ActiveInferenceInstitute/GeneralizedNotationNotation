...
generated from the producer's token map at commit 9c3f0c9fc. A
...
\protect\breaktt{src/gnn/STEP\_INDEX.md} at commit 9c3f0c9fc; its footer
...
producer tokens at commit 9c3f0c9fc, and the panel is generated by
...
model-family manifest at commit 9c3f0c9fc, and its footer summarizes the
...
repository state at commit 9c3f0c9fc, not typed by hand. The producer
...
state at commit 9c3f0c9fc, so each figure here is regenerated from the
...
9c3f0c9fc.}\label{fig:family_matrix}
\end{figure}
...
9c3f0c9fc --- the same token map that substitutes the prose counts, so
...
The test suite comprises 456 test files containing 4890 test functions,
exercising a source base of 680 Python files across 44 packages (197836
lines of source). The Model Context Protocol surface --- which exposes
GNN's capabilities to external agents and tools --- provides 142 tools
...
\breaktt{output/data/manuscript\_variables.json} (9c3f0c9fc): a code
change that alters, for example, the test inventory (456 test files,
4890 test functions) propagates into the prose on the next regeneration
...
into 44 Python packages spanning 680 source files and roughly 197836
