...
repository state at commit 999c14ced, not typed by hand. The producer
...
state at commit 999c14ced, so each figure here is regenerated from the
...
directly from the tracked files at commit 999c14ced.
...
The test suite comprises 452 test files containing 4829 test functions,
exercising a source base of 672 Python files across 44 packages (197021
...
\breaktt{output/data/manuscript\_variables.json} (999c14ced): a code
change that alters, for example, the test inventory (452 test files,
4829 test functions) propagates into the prose on the next regeneration
...
into 44 Python packages spanning 672 source files and roughly 197021
