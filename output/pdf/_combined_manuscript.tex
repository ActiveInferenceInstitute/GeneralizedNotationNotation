...
purposes, read from
\protect\breaktt{src/gnn/STEP\_INDEX.md}.}\label{tbl:pipeline_steps}\tabularnewline
\toprule\noalign{}
...
\protect\breaktt{src/gnn/render/framework\_registry.py}. The
\emph{Executes} column is the registry's own
\protect\breaktt{supports\_execution} flag: a render-only backend has no
Step-12 executor.}\label{tbl:backend_registry}\tabularnewline
\toprule\noalign{}
...
repository state at commit d3c391a78, not typed by hand. The producer
...
state at commit d3c391a78, so each figure here is regenerated from the
...
not authored in the manifest.}\label{tbl:model_families}\tabularnewline
\toprule\noalign{}
...
directly from the tracked files at commit d3c391a78.
...
The test suite comprises 450 test files containing 4808 test functions,
exercising a source base of 672 Python files across 44 packages (196624
...
across 32 modules. The pipeline itself runs as 25 steps (0--24), and 6
...
\breaktt{output/data/manuscript\_variables.json} (d3c391a78): a code
change that alters, for example, the test inventory (450 test files,
4808 test functions) propagates into the prose on the next regeneration
...
into 44 Python packages spanning 672 source files and roughly 196624
...
dumps, intermediate representations, validation reports, and the 6
