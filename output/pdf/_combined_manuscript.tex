...
and 141 Model Context Protocol tools.}\label{fig:graphical_abstract}
\end{figure}
...
round-trip faithfully across formats and execute across multiple
...
why a standard notation is needed, through how GNN realizes it, to the
evidence that the standard holds.
...
\protect\breaktt{src/gnn/STEP\_INDEX.md} at commit 0a12bb0b6; its footer
...
producer tokens at commit 0a12bb0b6, and the panel is generated by
...
organized into 11 curated corpora, ranging from minimal perception
...
backend. Validation also reads each declared \texttt{B} tensor through
an orientation diagnostic: the canonical reading is column-stochastic,
...
per tensor. Validation here is the gate that protects every later stage:
...
model-family manifest at commit 0a12bb0b6, and its footer summarizes the
...
have an executor at this step. The bnlearn lane is generator-backed: it
renders a runnable Python program from the same parsed model the other
lanes consume, and it skips with a recorded status when the
\texttt{bnlearn} package is absent, so a lean checkout degrades
explicitly rather than failing; skipped and degraded backends are
likewise recorded as explicit statuses in the cross-framework comparison
rather than silently omitted. A separate execute-side bridge verifies
rendered Lean documents against the fep\_lean bridge contract, which is
how the notation reaches a proof assistant without a render-registry
entry. This closes the loop from text to running cognitive model: the
same specification that was type-checked and exported is now exercised
...
repository state at commit 0a12bb0b6, not typed by hand. The producer
...
state at commit 0a12bb0b6, so each figure here is regenerated from the
...
0a12bb0b6.}\label{fig:family_matrix}
\end{figure}
...
tracked gridworld, a rectangular Gaussian, and an explicit factored
fixture --- inside the GEO environment, writing its receipts even on
failure. Like the gates above it is stated as a command, not as a pass
count, and its pinned pair (\breaktt{ActiveInferenceInstitute/GEO-INFER}
at a recorded revision) is what makes ``interchange'' a checkable claim
...
0a12bb0b6 --- the same token map that substitutes the prose counts, so
...
The test suite comprises 455 test files containing 4875 test functions,
exercising a source base of 679 Python files across 44 packages (197628
...
across 32 modules. The pipeline itself runs as 25 steps (0--24), and 7
...
\breaktt{output/data/manuscript\_variables.json} (0a12bb0b6): a code
change that alters, for example, the test inventory (455 test files,
4875 test functions) propagates into the prose on the next regeneration
...
into 44 Python packages spanning 679 source files and roughly 197628
...
files. Every model file under \texttt{input/} lives in that subtree, so
the 30-file count covers the whole tree. A
...
section hard-codes one instead, and the resulting 86-entry map is
...
with the meaning each carries. The first six rows are the required
sections that pin a model's identity, variables, and factor graph; the
optional sections that follow carry parameterization, ontology bindings,
timing, equations, and provenance. The three operator rows are the
connection vocabulary the \protect\texttt{Connections} section is
written in --- directed \protect\texttt{A\textgreater{}B}, undirected
\protect\texttt{A-B}, and their labeled v1.1 forms --- and
\protect\texttt{default=…} supplies initialization hints. Definitions
follow the GNN syntax specification and the discrete POMDP exemplars
...
step runs.}\label{tbl:gnn_constructs}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Construct
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.5000}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.5000}}@{}}
\caption{The Active Inference symbols a GNN specification carries, each
bound to the equation of sec.~\ref{sec:system_context} that fixes its
role. Read \protect\texttt{A}--\protect\texttt{E} as the
generative-model tensors (likelihood, controlled transitions,
log-preferences, prior over initial states, habit prior),
\protect\texttt{s}, \protect\texttt{o}, \protect\texttt{\texorpdfstring{\ensuremath{\pi}}{pi}}, and
\protect\texttt{u} as the inference variables, and \protect\texttt{F}
and \protect\texttt{G} as the two free-energy functionals minimized at
eq.~\ref{eq:vfe} and eq.~\ref{eq:efe}. Use the table as a decoding key
when reading a \protect\texttt{StateSpaceBlock}: every declared matrix
name should map to exactly one row here, and the
\protect\breaktt{\#\#\ ActInfOntologyAnnotation} bindings described below
are what make that mapping machine-checkable
...
PyTorch, NumPyro, Stan, bnlearn), of which the 9 listed as executable
(PyMDP, RxInfer.jl, ActiveInference.jl, JAX, DisCoPy, PyTorch, NumPyro,
Stan, bnlearn) also run at Step 12 --- so that a model written once in
