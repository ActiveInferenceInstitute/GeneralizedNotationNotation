...
repository state at commit d4c6f248f, not typed by hand. The producer
...
state at commit d4c6f248f, so each figure here is regenerated from the
...
directly from the tracked files at commit d4c6f248f.
...
The test suite comprises 367 test files containing 4133 test functions,
exercising a source base of 582 Python files across 31 packages (194808
...
10 corpus directories. All 9 registered family target directories lie
...
\breaktt{output/data/manuscript\_variables.json} (d4c6f248f): a code
change that alters, for example, the test inventory (367 test files,
4133 test functions) propagates into the prose on the next regeneration
...
into 31 Python packages spanning 582 source files and roughly 194808
...
files. Outside that subtree, \breaktt{input/multi\_agent\_models/} holds
1 model file and is registered by no manifest family;
\breaktt{input/recursive\_models/} holds no model files and is registered
by no manifest family. That model file is outside the 29-file count
above. A \breaktt{model\_family\_manifest.json} enumerates the 9
registered families and their target directories. This manifest is the
