...
Because the variables file is regenerated before rendering, the counts in the rendered PDF track the repository state at the commit recorded in `output/data/manuscript_variables.json` (0a12bb0b6): a code change that alters, for example, the test inventory (455 test files, 4875 test functions) propagates into the prose on the next regeneration without any manual editing.
