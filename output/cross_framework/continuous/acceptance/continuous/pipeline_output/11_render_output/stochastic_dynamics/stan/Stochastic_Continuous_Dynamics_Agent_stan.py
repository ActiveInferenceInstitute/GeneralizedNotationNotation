#!/usr/bin/env python3
"""
Stan continuous (linear-Gaussian state-space) driver: Stochastic Continuous Dynamics Agent
...
STAN_FILE = Path(__file__).resolve().with_name('Stochastic_Continuous_Dynamics_Agent_stan.stan')
RANDOM_SEED = 42
T = 15
F = np.asarray([[1.0, 0.1], [0.0, 0.9]], dtype=float)
H = np.asarray([[1.0, 0.0], [1.0, 0.0]], dtype=float)
Q = np.asarray([[0.1, 0.0], [0.0, 0.1]], dtype=float)
R = np.asarray([[0.2, 0.0], [0.0, 0.2]], dtype=float)
PRIOR_MEAN = np.asarray([0.0, 0.0], dtype=float)
PRIOR_COV = np.asarray([[0.5, 0.0], [0.0, 1.0]], dtype=float)
GOAL_MEAN = None
CONTROL_GAIN = None
