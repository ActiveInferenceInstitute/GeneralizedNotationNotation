...
(sec.~3.2) rather than discovered mid-run.

A third limitation is one of scope rather than capability. The formal
account in sec.~3 fixes each kind's
...
kind (eq.~6, eq.~7) and fixed-parameter filtering
...
\begin{frame}[allowframebreaks]{Next Steps}
proportion to the reviewability of what it does. The next-release items
are not claimed as complete here; they are the recorded direction of
travel, and each will be evidenced by its own gates when it lands, just
as the 3.0.0 contracts are evidenced by theirs today. On the model-kind
axis the same staging applies: extending native hierarchical rendering
beyond the two-level strategies, broadening continuous reach across the
remaining registered backends, and populating the reserved recursive
corpus each arrive with their own gate evidence rather than by prose
assertion.
\end{frame}
