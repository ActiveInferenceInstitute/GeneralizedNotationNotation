...
sec.~3.2: minimal perception fixtures used to test the
...
verbatim, the kind classification of sec.~3.2 is
...
\begin{frame}[fragile,allowframebreaks]{Type Checking and Validation}
...
the pymdp convention (Heins et al. 2022) and the transition semantics
fixed by eq.~3, and the check records every tensor's
...
Gaussian shape contract of eq.~9 and
eq.~10: \texttt{prior\_mean} must carry exactly as many
...
table against tbl.~5, which shows which families
...
Read with tbl.~1, which groups the same flags by
...
\begin{frame}[fragile,allowframebreaks]{Rendering by Model Kind}
\protect\phantomsection\label{rendering-by-model-kind}
...
sec.~5 anchors on exactly this breadth.
...
when the closed-loop declarations of eq.~11 are
...
notation declares, whether that is categorical free-energy minimization
(Da Costa et al. 2020; Smith et al. 2022) or Gaussian filtering and
control --- is the common substrate the renderer attempts to preserve;
where a backend cannot express a kind faithfully, GNN records that
boundary as an explicit unsupported status rather than treating all
registered backends as equivalent.
\end{frame}

\subsection{Execution}\label{execution}

\begin{frame}[fragile,allowframebreaks]{Executing by Model Kind}
\protect\phantomsection\label{executing-by-model-kind}
...
\begin{frame}[fragile,allowframebreaks]{Long-Running Orchestration
Contracts}
...
\begin{frame}[fragile,allowframebreaks]{Analysis, Visualization, and
Reporting}
...
manuscript injects: the model-kind table (tbl.~1)
