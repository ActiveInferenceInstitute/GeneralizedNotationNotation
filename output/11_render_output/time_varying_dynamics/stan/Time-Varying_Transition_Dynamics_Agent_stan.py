#!/usr/bin/env python3
"""
Stan POMDP/HMM driver: Time-Varying Transition Dynamics Agent
...
STAN_FILE = Path(__file__).resolve().with_name('Time-Varying_Transition_Dynamics_Agent_stan.stan')
RANDOM_SEED = 42
T = 10
S, O, U = 3, 3, 2
PSEUDO_COUNT_STRENGTH = 20.0
NUTS_PARAM_BUDGET = 1024
A = np.asarray([[0.85, 0.1, 0.05], [0.1, 0.8, 0.1], [0.05, 0.1, 0.85]], dtype=float)          # O x S, columns sum to 1
B = np.asarray([[[0.6000000000000001, 0.1], [0.30000000000000004, 0.1], [0.1, 0.6666666666666667]], [[0.30000000000000004, 0.1], [0.6000000000000001, 0.6], [0.1, 0.24999999999999997]], [[0.1, 0.8], [0.1, 0.3], [0.8, 0.08333333333333334]]], dtype=float)          # S(next) x S(prev) x U
D = np.asarray([0.33, 0.33, 0.34], dtype=float)          # S
