#!/usr/bin/env python3
"""
Stan POMDP/HMM driver: Static Perception Model
...
STAN_FILE = Path(__file__).resolve().with_name('Static_Perception_Model_stan.stan')
RANDOM_SEED = 42
T = 5
S, O, U = 2, 2, 2
PSEUDO_COUNT_STRENGTH = 20.0
NUTS_PARAM_BUDGET = 1024
A = np.asarray([[0.8181818181818181, 0.11111111111111112], [0.18181818181818182, 0.888888888888889]], dtype=float)          # O x S, columns sum to 1
B = np.asarray([[[0.95, 0.05], [0.05, 0.95]], [[0.05, 0.95], [0.95, 0.05]]], dtype=float)          # S(next) x S(prev) x U
