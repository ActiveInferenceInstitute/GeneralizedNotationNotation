#!/usr/bin/env python3
"""
Stan POMDP/HMM driver: Multi Armed Bandit Agent
...
STAN_FILE = Path(__file__).resolve().with_name('Multi_Armed_Bandit_Agent_stan.stan')
RANDOM_SEED = 42
T = 30
S, O, U = 3, 3, 3
PSEUDO_COUNT_STRENGTH = 20.0
NUTS_PARAM_BUDGET = 1024
A = np.asarray([[0.1, 0.5, 0.5], [0.3, 0.4, 0.3], [0.6, 0.1, 0.2]], dtype=float)          # O x S, columns sum to 1
B = np.asarray([[[0.9, 0.9, 0.9], [0.05, 0.05, 0.05], [0.05, 0.05, 0.05]], [[0.05, 0.05, 0.05], [0.9, 0.9, 0.9], [0.05, 0.05, 0.05]], [[0.05, 0.05, 0.05], [0.05, 0.05, 0.05], [0.9, 0.9, 0.9]]], dtype=float)          # S(next) x S(prev) x U
