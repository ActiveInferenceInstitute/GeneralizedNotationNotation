#!/usr/bin/env python3
"""
Stan POMDP/HMM driver: Dirichlet Likelihood Learning Agent
...
STAN_FILE = Path(__file__).resolve().with_name('Dirichlet_Likelihood_Learning_Agent_stan.stan')
RANDOM_SEED = 42
T = 15
S, O, U = 3, 3, 2
PSEUDO_COUNT_STRENGTH = 20.0
NUTS_PARAM_BUDGET = 1024
A = np.asarray([[0.85, 0.05, 0.1], [0.1, 0.9, 0.05], [0.05, 0.05, 0.85]], dtype=float)          # O x S, columns sum to 1
B = np.asarray([[[0.1, 0.9], [0.0, 0.05], [0.9, 0.05]], [[0.9, 0.05], [0.1, 0.9], [0.0, 0.05]], [[0.0, 0.05], [0.9, 0.05], [0.1, 0.9]]], dtype=float)          # S(next) x S(prev) x U
D = np.asarray([1.0, 0.0, 0.0], dtype=float)          # S
