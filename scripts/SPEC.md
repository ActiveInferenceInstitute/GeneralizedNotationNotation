...
3. `run_pymdp_gnn_scaling_analysis.py`: End-to-end thin orchestrator executing parameterized PyMDP scaling studies through Steps 3, 5, 11, 12, and 17 of `src/gnn/main.py` (Step 5 `type_check` is inserted after Step 3 by default; `--no-type-check` or config `type_check: false` drops it).
