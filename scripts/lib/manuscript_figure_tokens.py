...
Recording is observed, not declared: the keys come from real ``__getitem__``
calls, so a generator cannot claim to consume a token it never read, or quietly
read one it did not declare.
