...
def _check_streams(g: _Gate, workdir: Path, inject_defect: bool) -> None:
    ...
    g.expect(
        "streams: array manifest valid",
        ds.validate_stream_manifest(manifest, workdir) == [],
    )
    ...
    g.expect(
        "streams: verify_replay accepts true digest", ds.verify_replay(trace, digest)
    )
    g.expect(
        "streams: verify_replay rejects wrong digest",
        not ds.verify_replay(trace, "0" * 64),
    )
    ...
    bad = ds.ExecutionTrace(
        ...
        events=[
            ds.TraceEvent(
                seq=0, step="3", action="parse", payload_ref="", payload_checksum="x"
            ),
            ds.TraceEvent(
                seq=2,
                step="5",
    ...
    g.expect(
        "streams: NEGATIVE control fires on seq gap", len(ds.trace_integrity(bad)) > 0
    )
...
def _check_session(g: _Gate, workdir: Path) -> None:
    session = rs.start_session(
        "accept-1", ["basics", "discrete", "gridworld"], created_by="acceptance"
    )
    session = rs.mark(
        session, "basics", rs.UnitStatus.DONE, artifact_refs=["basics/out.json"]
    )
    session = rs.mark(
        session, "discrete", rs.UnitStatus.DONE, artifact_refs=["discrete/out.json"]
    )
    ...
    g.expect(
        "session: resume targets the unfinished unit",
        rs.remaining_units(loaded) == ["gridworld"],
    )
    report = rs.status_report(loaded)
    g.expect(
        "session: status math (2/3 done)",
        report["completed"] == 2 and not report["done"],
    )
    ...
    session = rs.mark(
        session,
        "gridworld",
        rs.UnitStatus.RUNNING,
        artifact_refs=["gridworld/out.json"],
    )
    removed = rs.cancel_safe_cleanup(session, workdir)
    g.expect(
        "session: cleanup removed the partial artifact",
        any("gridworld" in r for r in removed),
    )
    g.expect(
        "session: cleanup preserved DONE artifact",
        (workdir / "basics" / "out.json").exists(),
    )
    g.expect(
        "session: cleanup idempotent", rs.cancel_safe_cleanup(session, workdir) == []
    )
...
def _check_container(g: _Gate) -> None:
    ...
    g.expect(
        "container: hardened default passes security review",
        cp.security_review(plan) == [],
    )
    g.expect(
        "container: plan hash is deterministic",
        cp.compute_plan_hash(plan) == plan.plan_hash,
    )
    ...
    v2 = cp.generate_container_plan(
        ...
        [
            {
                "name": "runner",
                "image": "ghcr.io/gnn/runner@sha256:" + "b" * 64,
                "command": ["python", "src/main.py"],
            }
        ],
        previous=plan,
    ...
    g.expect(
        "container: v2 carries rollback to v1",
        v2.version == 2
        and v2.rollback is not None
        and v2.rollback.previous_plan_hash == plan.plan_hash,
    )
    ...
    g.expect(
        "container: NEGATIVE control flags insecure plan (incl. CRITICAL)",
        has_critical and len(codes) >= 4,
...
def main() -> int:
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="(default behavior) fail closed on any miss",
    )
    parser.add_argument(
        "--inject-defect",
        action="store_true",
        help="break a positive check to show non-zero exit",
    )
    ...
    print(
        "\n"
        + (
            "✅ v3.0.0 acceptance PASSED"
            if code == 0
            else "❌ v3.0.0 acceptance FAILED"
        )
    )
    return code
