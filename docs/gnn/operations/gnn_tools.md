...
Step 11 renders to ten backends and Step 12 executes all ten (bnlearn via `src/gnn/execute/bnlearn/`, skipping until its runtime is installed). `uv run gnn health` reports which are importable in the current environment.
