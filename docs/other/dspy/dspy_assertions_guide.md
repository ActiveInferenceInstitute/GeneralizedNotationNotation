...
class ConstrainedGenerator(dspy.Module):
    ...
    def forward(self, topic):
        ...
        dspy.Assert(
            len(result.summary.split()) <= 100, "Summary must be 100 words or fewer"
        )
...
class GuidedGenerator(dspy.Module):
    ...
    def forward(self, topic):
        ...
        dspy.Suggest(
            "example" in result.explanation.lower()
            or "for instance" in result.explanation.lower(),
...
class SafeContentGenerator(dspy.Module):
    ...
    def forward(self, topic):
        ...
        found_forbidden = [
            word for word in self.FORBIDDEN_WORDS if word in result.content.lower()
        ]
...
class GNNVariableGenerator(dspy.Module):
    ...
    def forward(self, description, existing_variables):
        result = self.generate(
            component_desc=description, existing_vars=existing_variables
        )
...
class ConstrainedSummarizer(dspy.Module):
    ...
    def forward(self, article):
        ...
        dspy.Suggest(result.summary.endswith("."), "Summary should end with a period")
...
dspy.Assert(simple_constraint, message, max_retries=2)
```
