...
| Wave2 pipeline + run identity | pytest over `src/tests/pipeline/test_wave2_sessions.py`, `test_wave2_identity.py`, `test_wave2_manifests.py`, `test_wave2_container_plans.py`, `test_run_identity.py` | `68 passed in 1.88s` — 0 failed, 0 skipped |
| LLM sync wrappers | pytest over `src/tests/llm/test_openai_sync_analysis_contract.py`, `test_llm_sync_wrappers.py` | `13 passed in 1.20s` — 0 failed, 0 skipped, no network/Ollama needed |
...
| Full suite (ground truth) | command of record: `uv run --extra dev python -m pytest src/tests/ -q --tb=no -rsx --ignore=src/tests/llm/test_llm_ollama.py --ignore=src/tests/llm/test_llm_ollama_integration.py` (run exactly once) | `4693 passed, 14 skipped in 536.44s (0:08:56)` — 0 failed, no collection errors |

Full-suite skips, all within the zero-skip allowlist: 11 sklearn
(`src/tests/ml_integration/test_ml_integration_inference.py`), 1 torch
(`src/tests/render/test_continuous_renderers.py:109`), 2 D2 CLI
(`src/tests/visualization/test_d2_visualizer.py:262,384`). The suite total
...
- Zero-skip truth: the only `importorskip` occurrences under `src/tests/`
...
- Security surface: `src/export/processor.py` reads XML/GraphML/GEXF via
  ...
  `src/export/format_exporters.py` or `formatters.py` (stdlib ElementTree is
  used for serialization only). `src/ml_integration/inference.py` implements
...
- OpenAI sync wrapper: `src/llm/providers/openai_provider.py` `analyze` is
...
- Hygiene: zero TODO/FIXME/XXX in the wave2-owned files (`src/main.py`,
  `src/cli/__init__.py`, `src/pipeline/*.py`).
...
3. Stale dependency lists: `src/export/AGENTS.md` and
   `doc/gnn/modules/07_export.md` Required Dependencies omitted the hardened
   reader; both now list `defusedxml` (mirroring `src/export/README.md`,
   which already documented it).

Awareness, deliberately not changed under the docs-only mandate:
- `src/export/formatters.py` keeps a stdlib `xml.dom.minidom` fallback on
...
- `src/ml_integration/AGENTS.md` lists `pickle` as a stdlib dependency, which
  is accurate; the restricted-loader behavior is documented in
  `src/ml_integration/README.md` (allowlist, extension opcodes, trailing
...
- Consider dropping the dead `xml.dom.minidom` ImportError fallback in
  `src/export/formatters.py` when a code window opens.
...
- This session's footprint (verified): 4 tracked doc edits (`README.md`,
  `CHANGELOG.md`, `src/export/AGENTS.md`, `doc/gnn/modules/07_export.md`) plus
...
- Concurrent-lane activity dated 2026-09-05, none of it this session's: a
  ...
  09:08–09:09; `src/manuscript_variables.py`) and an MCP audit regeneration
  (`src/mcp/audit_report.json`, 00:42). This is a shared fleet checkout.
...
- Directory-ownership rule adopted for any follow-on work in this checkout:
  ...
  `src/manuscript_variables.py`, and `src/mcp/audit_report.json` belong to the
