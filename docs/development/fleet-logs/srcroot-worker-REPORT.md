...
Worker scope: `src/gnn/main.py`, `src/gnn/manuscript_variables.py`, `src/gnn/__init__.py`,
`src/gnn/README.md`, `src/gnn/SPEC.md`, `src/gnn/STEP_INDEX.md`, `src/gnn/AGENTS.md`,
`src/gnn/doc/`. Branch main @ f64ac9085 (start). No git mutations performed.

## Files changed + why

| File | Change |
|---|---|
| `src/gnn/main.py` | Composability refactor (see API deltas). 1734→~1730 lines. Step selection extracted into a pure typed core; serial/parallel result recording deduplicated into `_record_step_result`; YAML config loading deduplicated into `_read_input_config`; startup errors now fail through the pipeline exit contract. |
| `src/gnn/manuscript_variables.py` | `generate_variables` inline table loops extracted into four pure helpers (`_render_step_table`, `_render_family_table`, `_render_backend_table`, `_cross_framework_selection`) — byte-identical output. Added `load_variables` + `token_checksum` (exported). Public names `generate_variables`/`save_variables` and all token keys unchanged. |
| `src/gnn/__init__.py` | `__all__` typed `list[Any]` → `list[str]`. No API change. |
| `src/gnn/AGENTS.md` | New "Programmatic step selection" section documenting the selection API + fail-fast behavior; Last Updated → 2026-09-04. |
| `src/gnn/README.md` | Fail-fast step-selection note under "Running the pipeline". |
| `src/gnn/SPEC.md` | "Behavior" bullet documenting invalid-selection fail-fast semantics. |
| `tests/main/` (new) | `__init__.py`, `AGENTS.md`, `README.md`, `test_main_step_selection.py` (18 tests), `test_manuscript_variables_api.py` (6 tests). |
| `src/gnn/STEP_INDEX.md`, `src/gnn/doc/` | Unchanged — no API/behavior surface they document changed. |
...
Consumer safety: verified by grep — `main()` consumers (`src/gnn/cli/__init__.py`,
`src/gnn/pipeline/execution.py` via `execute_pipeline_step`, tests pinning
...
python src/gnn/main.py --help ................................... help_exit=0 (usage unchanged)
pytest tests/main/ tests/test_manuscript_variables.py -q
............................................................. 36 passed in 0.42s
pytest tests/pipeline/test_main_orchestrator.py -m unit -q
............................................................. 8 passed, 17 deselected
mypy src/gnn/main.py src/gnn/manuscript_variables.py --config-file pyproject.toml
...
- New test files linted with the repo gate surface: `ruff check
  tests/main/` → clean; `ruff format tests/main/` applied (2 files
 ...
 - Transient fleet collisions observed (both self-healed by peers, no action
  taken): `src/gnn/pipeline/__init__.py` briefly missing
