...
| COMP-004 | MEDIUM | `src/gnn/cli/__init__.py:1012-1013` | Private cross-module import: the reproduce command reaches into `gnn.main` internals (`_resolve_steps_to_execute`) | Promote to a public API with a contract test |
| COMP-005 | MEDIUM | `src/gnn/api/parity.py:458,522,647,757,856` | Parity by mirror: the API re-states `gnn.cli._cmd_*` severity and payload checks in five handlers | Extract the mirrored check bodies into shared pure helpers both surfaces consume |
| COMP-006 | MEDIUM | `src/gnn/utils/__init__.py:280-281` vs `src/gnn/export/core.py:10`, `src/gnn/audio/sapf/generator.py:10`, `src/gnn/pipeline/pipeline_step_template.py:46` | Dual canonical path for `get_output_dir_for_script` and `execute_pipeline_step_template` (facade map AND direct `gnn.pipeline` import) | Single path = `gnn.pipeline`; drop the facade re-exports; update the facade golden list (`tests/tests/test_infrastructure_exports.py`) |
