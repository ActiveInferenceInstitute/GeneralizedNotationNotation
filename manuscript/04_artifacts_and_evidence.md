...
This section reports what the project has actually produced and how each quantitative claim is grounded. Every number below is substituted at render time from the deterministic producer, which reads the repository state at commit {{GNN_GIT_COMMIT}}, so each figure here is regenerated from the artifacts it describes rather than transcribed.

## Model-Family Coverage

GNN ships a curated corpus of model families that exercise the language across the difficulty gradient from minimal parser fixtures to full multi-agent and scaling studies. The manifest registers {{GNN_FAMILY_COUNT}} families ({{GNN_FAMILY_LIST}}) across {{GNN_FAMILY_TARGET_DIR_COUNT}} target directories, and `input/gnn_files` holds {{GNN_INPUT_FAMILY_DIR_COUNT}} corpus directories containing {{GNN_EXAMPLE_COUNT}} concrete example models. The three sets are not coextensive, and the manuscript does not treat them as one. Registered in no family: {{GNN_UNREGISTERED_CORPUS_DIRS}} ({{GNN_UNREGISTERED_CORPUS_DIR_COUNT}} of the {{GNN_INPUT_FAMILY_DIR_COUNT}} corpus directories), covered in [@sec:limitations_next_steps]. Registered but outside the scanned tree: the `multiagent` family's target directory, whose model the gates exercise even though it is not among the {{GNN_EXAMPLE_COUNT}} models under `input/gnn_files`. Each family declares the simulation frameworks it is meant to drive, which is what lets the same text model fan out across the executable-model leg of the Triple Play [@gnn2023]. The families and their declared frameworks are enumerated below.
...
The cross-framework reliability gate, `scripts/run_cross_framework_reliability.py`, takes a single GNN model and renders it across multiple simulation backends, then checks that the resulting executable models agree on the structure they were generated from. The reference comparison runs on the {{GNN_CROSS_FRAMEWORK_FAMILY}} family across {{GNN_CROSS_FRAMEWORK_BACKENDS}} — {{GNN_CROSS_FRAMEWORK_BACKEND_COUNT}} independent Active Inference engines spanning the Python and Julia ecosystems [@heins2022]. The family declares {{GNN_CROSS_FRAMEWORK_DECLARED_BACKEND_COUNT}} backends ({{GNN_CROSS_FRAMEWORK_DECLARED_BACKENDS}}); {{GNN_CROSS_FRAMEWORK_UNPROFILED_BACKENDS}} is declared but not among the {{GNN_MAINTAINED_FRAMEWORK_COUNT}} frameworks the gate profiles, so it is excluded from the comparison. Because the same source model drives all {{GNN_CROSS_FRAMEWORK_BACKEND_COUNT}} renderings, disagreement between backends localizes a generator defect rather than a modeling choice.
