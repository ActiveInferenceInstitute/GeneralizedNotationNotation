...
**Test Suite Evidence (verified 2026-08-02)**: command of record is `uv run --extra dev python -m pytest src/tests/ -q --tb=no -rsx --ignore=src/tests/llm/test_llm_ollama.py --ignore=src/tests/llm/test_llm_ollama_integration.py`; latest local evidence is 2,632 passed, 0 failed, 0 skipped (Julia backend execution requires the packages in the project selected by `JULIA_PROJECT`, e.g. `JULIA_PROJECT=/tmp/julia_test_env` after `julia --project=/tmp/julia_test_env -e 'using Pkg; Pkg.add(["RxInfer","JSON","Distributions","StatsBase"])'`). With a local Ollama daemon and `smollm2:135m-instruct-q4_K_S` pulled, the two ignored files are re-enabled and the full suite is 2,658 passed, 0 failed.
