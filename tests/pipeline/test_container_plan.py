...
class TestGenerateAndReviewClean:
    def test_hardened_default_is_clean(self) -> Any:
        from gnn.pipeline.container_plan import generate_container_plan, security_review
    ...
    def test_hardened_defaults_applied(self) -> Any:
        from gnn.pipeline.container_plan import generate_container_plan
...
class TestSecurityReviewNegativeControl:
    ...
    def _insecure_plan(self) -> Any:
        from gnn.pipeline.container_plan import (
            ContainerPlan,
            ContainerSpec,
            ResourceLimits,
        )
    ...
    def test_each_issue_is_reported(self) -> Any:
        from gnn.pipeline.container_plan import security_review
...
class TestSerialize:
    def test_serialize_is_sorted_and_roundtrips(self) -> Any:
        from gnn.pipeline.container_plan import generate_container_plan, serialize_plan
    ...
    def test_to_compose_is_pure_data(self) -> Any:
        from gnn.pipeline.container_plan import generate_container_plan, plan_to_compose
...
class TestSafetyContract:
    def test_module_has_no_execution_path(self) -> Any:
        ...
        """
        from gnn.pipeline import container_plan
...
def test_unpinned_image_strict_digest_rejects_fake_digests() -> None:
    ...
    for bad_image in (
        ...
        "x@sha256:" + ("0" * 64),  # all-zero sentinel is NOT a real pin
