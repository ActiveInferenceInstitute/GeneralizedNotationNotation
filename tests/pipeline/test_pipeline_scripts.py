...
class TestPipelineScriptDiscovery:
    ...
    def test_all_pipeline_scripts_exist(self) -> None:
        ...
        missing_scripts = [
            (num, name) for num, name in expected_scripts if num not in existing_nums
        ]
        if missing_scripts:
        ...
        assert len(existing_scripts) >= 5, (
            f"Expected at least 5 pipeline scripts, found {len(existing_scripts)}"
        )
    ...
    def test_pipeline_script_structure(self, script_name: str) -> None:
        ...
        assert content.startswith("#!/usr/bin/env python3"), (
            f"Script {script_name} should start with shebang"
        )
        ...
        assert has_main_func or has_main_execution, (
            f"Script {script_name} should have main function or main execution block"
        )
        ...
        has_argparse = (
            ...
            or "ArgumentParser" in content
            or ("parse_args" in content)
            or ("add_argument" in content)
            or ("sys.argv" in content)
            or ("create_standardized_pipeline_script" in content)
        )
...
class TestPipelineScriptExecution:
    ...
    @pytest.mark.slow
    @pytest.mark.parametrize(
        "script_name,artifact_checker",
        [
            ("3_gnn.py", lambda outdir: (outdir / "3_gnn_output").exists()),
            (
                "5_type_checker.py",
                lambda outdir: (
                    (outdir / "5_type_checker_output").exists()
                    or (
                        outdir / "5_type_checker_output" / "type_check_results.json"
                    ).exists()
                ),
            ),
            ...
            (
                "8_visualization.py",
                lambda outdir: (outdir / "8_visualization_output").exists(),
            ),
        ],
    )
    def test_script_executes_real(
        self, script_name: str, artifact_checker: Any
    ) -> None:
        script_path = SRC_DIR / script_name
        if not script_path.exists():
            raise AssertionError(f"Script {script_name} not found")
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            assert artifact_checker(output_dir), (
                f"Expected artifacts not produced by {script_name}"
            )
...
class TestStep2GNNComprehensive:
    ...
    @pytest.mark.unit
    def test_step2_gnn_file_discovery(
        ...
        for file_path in gnn_files[:2]:
            ...
            assert parsed.get("success") is True, (
                f"{file_path.name}: parse unsuccessful: {parsed.get('errors')}"
            )
    ...
    def test_step2_gnn_validation(self, sample_gnn_files: Any) -> None:
        ...
        for file_path in sample_gnn_files.values():
            ...
            assert "valid" in result, (
                f'{file_path.name}: missing "valid" key in {list(result.keys())}'
            )
...
class TestStep1SetupComprehensive:
    ...
    def test_step1_environment_validation(self) -> None:
        ...
        from gnn.setup import check_uv_availability, validate_environment
...
class TestStep5ExportComprehensive:
    ...
    @pytest.mark.unit
    def test_step5_export_formats(
        self, sample_gnn_files: Any, isolated_temp_dir: Any
    ) -> None:
        """JSON + XML exporters write files to disk from a parsed GNN spec."""
        from gnn.export import export_to_json, export_to_xml
...
class TestStep6VisualizationComprehensive:
    ...
    @pytest.mark.unit
    def test_step6_visualization_generation(
        self, sample_gnn_files: Any, isolated_temp_dir: Any
    ) -> None:
...
class TestStep8OntologyComprehensive:
    ...
    def test_step8_ontology_processing(self) -> None:
        ...
        from gnn.ontology import load_defined_ontology_terms, parse_gnn_ontology_section

        terms = load_defined_ontology_terms()
        assert isinstance(terms, dict), (
            "load_defined_ontology_terms should return a dict"
        )
        ...
        parsed = parse_gnn_ontology_section(
...
class TestStep9RenderComprehensive:
    ...
    @pytest.mark.unit
    def test_step9_code_rendering(
        self, sample_gnn_files: Any, isolated_temp_dir: Any
    ) -> None:
        """PyMDP and RxInfer renderers are importable + callable (real signature)."""
        from gnn.render import render_gnn_to_pymdp, render_gnn_to_rxinfer
...
class TestStep10ExecuteComprehensive:
    ...
    def test_step10_execution_safety(self) -> None:
        ...
        from gnn.execute import execute_script_safely, validate_execution_environment

        env_valid = validate_execution_environment()
        assert isinstance(env_valid, dict), (
            f"validate_execution_environment should return dict, got {type(env_valid).__name__}"
        )
...
class TestStep11LLMComprehensive:
    ...
    def test_step11_llm_operations(self) -> None:
        ...
        from gnn.llm import LLMProcessor

        processor = LLMProcessor()
        # analyze_model takes either a dict or a raw content string.
        analysis = processor.analyze_model(
            {"ModelName": "TestModel", "content": "## ModelName\nTestModel"}
        )
        assert isinstance(analysis, dict)
        description = processor.generate_description(
            "## ModelName\nTestModel\n## Connections\ns>o"
        )
...
class TestStep20WebsiteComprehensive:
    ...
    def test_step20_website_generation(self, isolated_temp_dir: Any) -> None:
        ...
        """
        from gnn.website import generate_html_report, generate_website
...
class TestStep15AudioComprehensive:
    ...
    @pytest.mark.unit
    def test_step15_audio_generation(
        self, sample_gnn_files: Any, isolated_temp_dir: Any
    ) -> None:
        """Test SAPF audio generation."""
        try:
            from gnn.audio.sapf.audio_generators import (
...
class TestStep14ReportComprehensive:
    ...
    @pytest.mark.unit
    def test_step14_report_generation(
        self, sample_gnn_files: Any, isolated_temp_dir: Any
    ) -> None:
    ...
    def test_pipeline_argument_consistency(self) -> None:
        ...
        for script_name in scripts:
            ...
            if has_argparse:
                ...
                logging.warning(
                    f"Script {script_name} has unclear argument handling pattern"
                )
...
def test_pipeline_script_completeness() -> None:
    ...
    assert len(found_scripts) >= 10, (
        f"Expected >=10 pipeline scripts, found {len(found_scripts)}"
    )
    logging.info(
        f"Pipeline script completeness: {len(found_scripts)}/{len(expected_scripts)} scripts found"
    )
