...
from gnn.llm import LLMAnalyzer, LLMProcessor, analyze_gnn_model, get_module_info
from gnn.llm.llm_processor import (
    GNNLLMProcessor,
    _error_result,
    enhance_model,
...
def _no_provider_init(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("gnn.llm.llm_processor.LLMProcessor.initialize", _fail)
...
class TestFacadeCompat:
    ...
    def test_get_module_info_providers_env_driven(
        ...
        monkeypatch.setattr(
            "gnn.llm.llm_processor.load_api_keys_from_env", lambda: {"ollama": "local"}
        )
