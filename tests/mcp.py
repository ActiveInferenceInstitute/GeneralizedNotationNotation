...
# Import utilities from the testing package public API
...
def run_all_tests_mcp(
    ...
    try:
        ...
        logger.error(
            f"Error in run_all_tests_mcp for {target_directory}: {e}", exc_info=True
        )
...
def run_unit_tests_mcp(output_directory: str, verbose: bool = False) -> Dict[str, Any]:
    ...
    try:
        success = run_test_category(
            "unit", test_results_dir=Path(output_directory), verbose=verbose
        )
        return {
            "success": success,
            "output_directory": output_directory,
        ...
        logger.error(
            f"Error in run_unit_tests_mcp for {output_directory}: {e}", exc_info=True
        )
        return {"success": False, "error": str(e)}


def run_integration_tests_mcp(
    target_directory: str, output_directory: str, verbose: bool = False
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.error(
            f"Error in run_integration_tests_mcp for {target_directory}: {e}",
            exc_info=True,
        )
        return {"success": False, "error": str(e)}
