...
# Run tests
pytest tests/ -v
pytest tests/test_gnn_*.py -v  # Module-specific tests
python src/gnn/2_tests.py --comprehensive  # Full test suite via pipeline
...
The canonical source of truth for all 25 steps is `src/gnn/pipeline/step_registry.py`
...
| Path | Purpose |
|------|---------|
| `src/gnn/pipeline/step_registry.py` | Canonical 25-step pipeline registry (single source of truth) |
| `src/gnn/main.py` | Main pipeline orchestrator |
| `src/N_*.py` | Pipeline step scripts (0-24) |
| `src/gnn/` | GNN parsing, discovery, validation |
| `src/gnn/render/` | Code generation for all frameworks |
| `src/gnn/execute/` | Simulation execution |
| `tests/` | Test suite (`uv sync --extra dev` then `uv run pytest tests/ -q --tb=no --ignore=tests/llm/test_llm_ollama.py --ignore=tests/llm/test_llm_ollama_integration.py`; use current run output for pass/skip counts; enable those files when local `ollama` is available) |
...
- Tests in `tests/test_{module}_*.py`
