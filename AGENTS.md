...
- **Tests**: command of record is `uv run --extra dev python -m pytest src/tests/ -q --tb=no -rsx --ignore=src/tests/llm/test_llm_ollama.py --ignore=src/tests/llm/test_llm_ollama_integration.py`; latest local evidence (2026-08-03) is 2,642 passed, 0 failed, 0 skipped. Julia RxInfer execution (including `test_gridworld_render_execute_analyze_visualize_strict`) uses the committed `Project.toml` under `src/execute/rxinfer/` (RxInfer 5.5.0 pinned) — run scripts with `julia --startup-file=no --project=src/execute/rxinfer <script>`. With a local Ollama daemon and `smollm2:135m-instruct-q4_K_S` pulled, the two Ollama files are re-enabled and the full suite is 2,668 passed, 0 failed.
...
- **Pipeline Rules**: [.agent_rules](.agent_rules)
