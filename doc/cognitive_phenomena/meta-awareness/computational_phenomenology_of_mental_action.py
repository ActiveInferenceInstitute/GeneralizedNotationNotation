...
C2 = np.array(
    [2, -2]
)  ### preference over attentional outcomes (agent is trying to perform the task - ie. maintain attention on the stimulous)
...
u2 = np.zeros(
    T
)  ### discrete generative process active states --> u2 sets transition probabilities for x2
...
B3[:, 0] = [
    0.9,
    0.1,
]  ###probability of high/low meta-awareness, given high meta-awareness
B3[:, 1] = [
    0.1,
    0.9,
]  ###probability of high/low meta-awareness, given low meta-awareness


B2a = np.zeros((2, 2))  ### maintain attentional state - "stay"
B2b = np.zeros((2, 2))  ### switch atttentional state - "switch"

B2a[:, 0] = [0.8, 0.2]  ###probability of focus/distracted, given focus+stay
...
for t in range(T):
    betaA1 = np.sum(
        betaA1m * np.inner(A2, X2[:, t])
    )  ### Bayesian model average (prior precision beliefs)
    gammaA1[t] = (
        betaA1m[int(x2[t])] ** -1
    )  ### actual precision is based on generative process (earlier it was betaA1**-1)

    A1bar = softmax_dim2(A1 ** gammaA1[t])  ### Precision weighted likelihood mapping
    O1[:, t] = np.inner(A1bar, X1[:, t])  ### Observation priors

    X1bar[:, t] = softmax(
        np.log(X1[:, t]) + gammaA1[t] * np.log(A1[int(O[t]), :])
    )  ## calculate perceptual state posterior
    ...
    for i in range(2):  ##loop over outcomes
        for j in range(2):  ##loop over states
            AtC += (
                (O1bar[i, t] - A1bar[i, j]) * X1bar[j, t] * np.log(A1[i, j])
    ...
    X2bar[:, t] = softmax(
        np.log(X2[:, t]) - 1.0 * np.log((betaA1m - AtC) / betaA1m * betaA1 / betaA1bar)
    ...
    if t < (T - 1):
        ...
        u2[t] = np.arange(2)[
            Pi2[:, t] == np.amax(Pi2[:, t])
        ]  ###generative process is discrete
        ...
        if fig7:
            ...
            if u2[t] == 0:
                x2[t + 1] = np.random.choice(
                    [0, 1], p=B2a[:, int(x2[t])]
                )  ###generate true state from action 0
            else:
                x2[t + 1] = np.random.choice(
                    [0, 1], p=B2b[:, int(x2[t])]
                )  ###generate true state from action 1

    X2abar = softmax(
        np.log(X2a) - 1.0 * np.log((betaA1m - AtC) / betaA1m * betaA1 / betaA1bar)
    )
    X2bbar = softmax(
        np.log(X2b) - 1.0 * np.log((betaA1m - AtC) / betaA1m * betaA1 / betaA1bar)
    )

    if t > 0:
        F2[0, t - 1] = np.sum(
            X2abar * (np.log(X2abar) - np.log(A2[int(x2[t]), :]) - np.log(X2a))
        )
        F2[1, t - 1] = np.sum(
            X2bbar * (np.log(X2bbar) - np.log(A2[int(x2[t]), :]) - np.log(X2b))
        )
...
plt.plot(
    1.5 + np.arange(0, T - 1), Pi2bar[0, :-1], label=r"${\bar{u}}^{(2)}$", color="coral"
)
...
O2bar = np.zeros(
    (2, T)
)  ### observation posterior on the second level (focused vs distracted)
...
for t in range(T):
    ...
    betaA2 = np.sum(
        betaA2m * np.inner(A3, X3[:, t])
    )  ### Bayesian model average (prior precision beliefs)
    gammaA2[t] = (
        betaA2m[int(x3[t])] ** -1
    )  ### actual precision is based on generative process (earlier it was betaA2**-1)
    A2bar = softmax_dim2(np.log(A2) * gammaA2[t])
    O2bar[int(x2[t]), t] = (
        1  ###set the true lower-level attentional states as observations to inform the meta-awareness charge
    )

    ###attentional level X2:
    betaA1 = np.sum(
        betaA1m * np.inner(A2bar, X2[:, t])
    )  ### Bayesian model average (prior precision beliefs)
    gammaA1[t] = (
        betaA1m[int(x2[t])] ** -1
    )  ### actual precision is based on generative process (earlier it was betaA1**-1)
    ...
    X2bar[:, t] = softmax(
        np.log(X2[:, t])
        + gammaA2[t] * np.log(A2[int(x2[t]), :])
        - 1.0 * np.log((betaA1m - AtC) / betaA1m * betaA1 / betaA1bar)
    )
    ...
    for i in range(2):  ##loop over outcomes
        for j in range(2):  ##loop over states
            MaC += (
                (O2bar[i, t] - A2bar[i, j]) * X2bar[j, t] * np.log(A2[i, j])
            )  ### See "Uncertainty, epistemics and active inference" Parr, Friston.
    if MaC > betaA2m[0]:
        MaC = betaA2m[0] - 10**-5
    ...
    X3bar[:, t] = softmax(
        np.log(X3[:, t])
        + 0.1 * np.log(A3[int(x3[t]), :])
        - 1.0 * np.log((betaA2m - MaC) / betaA2m * betaA2 / betaA2bar)
    )  ## calculate meta-awareness state posterior given precision beliefs
