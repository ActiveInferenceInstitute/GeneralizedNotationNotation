...
| Pass 2 | Historical run: `uv run --extra dev python -m pytest src/tests/ -q --tb=no --ignore=src/tests/llm/test_llm_ollama.py --ignore=src/tests/llm/test_llm_ollama_integration.py`; use current run output instead of the 2026-03-24 pass/skip totals |
