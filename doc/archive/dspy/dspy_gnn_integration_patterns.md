...
class GNNObservationProcessor(dspy.Module):
    ...
    def forward(self, user_input: str) -> ObservationSet:
        ...
        result = self.extract(
            user_input=user_input, observation_schema=str(self.schema)
        )
        ...
        for obs in obs_set.observations:
            validation = self.validate(
                observation=str(obs), valid_variables=list(self.schema.keys())
            )
...
class GNNGoalTranslator(dspy.Module):
    ...
    def forward(self, goal: str) -> GoalTranslation:
        result = self.translate(
            goal_description=goal, available_states=self.state_space
        )
...
class GNNPolicyEvaluator(dspy.Module):
    ...
    def evaluate(
        self, policy: Dict, trajectory: List[str], goal: str
    ) -> PolicyAnalysis:
    ...
    def rank_policies(self, policies: List[Dict], goal: str) -> int:
        ...
        result = self.compare_policies(policies=policy_descriptions, goal=goal)
...
analysis = evaluator.evaluate(
    policy, trajectory, "Find and reach the target efficiently"
)
...
class GNNAuthoringAssistant(dspy.Module):
    ...
    def refine(self, model: GNNModelDraft, feedback: str) -> GNNModelDraft:
        ...
        result = self.refine_model(current_model=model.to_gnn_code(), feedback=feedback)
...
refined = assistant.refine(
    model, "Add a state for whether the agent has found the goal"
)
```
...
class GNNExplainer(dspy.Module):
    ...
    def explain(
        ...
        state_explanation = self.explain_state(
            state_distribution=str(beliefs), state_meanings=str(state_meanings)
        ).interpretation

        # Explain action choice
        action_explanation = self.explain_action(
            chosen_action=action, efe_values=str(efe_values), goal=goal
        ).rationale
...
class GNNActiveInferenceAgent(dspy.Module):
    ...
    def __init__(self, gnn_engine, state_space: List[str]):
        ...
        self.goal_translator = dspy.ChainOfThought("natural_goal -> formal_preferences")
    ...
    def select_action(
        self, beliefs: Dict[str, float], goal: str
    ...
    def step(
        self, observation_text: str, goal: str, explain: bool = False
    ) -> ActiveInferenceStep:
...
def semantic_alignment_metric(example, prediction, trace=None):
    ...
    result = judge(
        expected=example.expected_behavior, actual=prediction.behavior_description
    )
...
optimizer = MIPROv2(metric=combined_gnn_metric, num_candidates=10)
...
optimized_processor = optimizer.compile(
    student=GNNObservationProcessor(schema), trainset=trainset
)
```
...
def validate_before_execution(gnn_code: str) -> bool:
    ...
    dspy.Assert(
        is_valid_gnn_syntax(gnn_code), f"Invalid GNN syntax. Check specification."
    )
    return True
```
