...
results = colbert(query="Expected Free Energy calculation", k=10)
...
class QdrantRetriever(dspy.Retrieve):
    ...
    def forward(self, query):
        ...
        results = self.client.search(
            collection_name=self.collection, query_vector=query_vector, limit=self.k
        )

        return [
            dspy.Passage(long_text=r.payload["text"], score=r.score) for r in results
        ]
...
class WeaviateRetriever(dspy.Retrieve):
    ...
    def forward(self, query):
        result = (
            self.client.query.get(self.class_name, ["content", "title"])
        ...
        return [
            dspy.Passage(long_text=p["content"], score=p["_additional"]["certainty"])
            for p in passages
        ]
```
...
class PineconeRetriever(dspy.Retrieve):
    ...
    def forward(self, query):
        ...
        results = self.index.query(
            vector=query_vector, top_k=self.k, include_metadata=True
        )

        return [
            dspy.Passage(long_text=match["metadata"]["text"], score=match["score"])
...
class MilvusRetriever(dspy.Retrieve):
    ...
    def forward(self, query):
        ...
        return [
            dspy.Passage(long_text=hit.entity.get("text"), score=hit.score)
...
class MultiHopRAG(dspy.Module):
    ...
    def forward(self, question):
        ...
        return self.generate_answer(question=question, context="\n\n".join(context))
```
...
class ReflectiveRAG(dspy.Module):
    ...
    def forward(self, question):
        ...
        for _ in range(self.max_refinements):
            critique = self.critique(question=question, answer=answer, context=context)
            ...
            # Refine answer
            answer = self.refine(
                question=question, previous_answer=answer, new_context=new_context
            ).refined_answer
...
class GNNDocumentationRetriever(dspy.Module):
    ...
    def forward(self, query):
        ...
        results = self.gnn_retriever(expanded.expanded_query, k=self.k)
...
class GNNModelExampleRetriever(dspy.Module):
    ...
    def __init__(self, model_index):
        ...
        self.analyze = dspy.ChainOfThought(
            "user_description -> model_type, state_space_type, search_query"
        )
    ...
    def forward(self, description):
        ...
        return {"analysis": analysis, "similar_models": examples}
```
...
def rag_metric(example, prediction, trace=None):
    ...
    key_info_found = any(key in prediction.answer.lower() for key in example.key_facts)

    # Check answer relevance
    relevance = len(
        set(prediction.answer.split()) & set(example.reference_answer.split())
...
optimized_rag = optimizer.compile(student=MultiHopRAG(num_hops=2), trainset=trainset)
```
...
class HierarchicalRetriever(dspy.Module):
    ...
    def forward(self, query, k=5):
        ...
        for doc_id in doc_ids[:10]:
            doc_passages = self.fine(f"{query} document:{doc_id}", k=5)
            passages.extend(doc_passages)

        # Rerank
        ranking = self.reranker(
            query=query, passages=[p["text"][:200] for p in passages]
        )
...
def evaluate_retrieval(retriever, eval_set):
    ...
    for example in eval_set:
        ...
        metrics["recall@k"].append(len(relevant & retrieved) / len(relevant))
        metrics["precision@k"].append(len(relevant & retrieved) / len(retrieved))
