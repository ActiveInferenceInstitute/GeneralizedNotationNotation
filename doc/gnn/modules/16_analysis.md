...
- `--advanced-stats` (store_true, default: off) — Registered CLI flag; **not consumed by the current `src/analysis/` implementation**, so it has no effect beyond parsed args.
...
### Test Coverage
- Measure: `uv run --extra dev python -m pytest src/tests/analysis/ --cov=analysis --cov-report=term-missing` (do not treat fixed percentages in this doc as canonical).
...
Registered in `register_tools` (`src/analysis/mcp.py`):
